...
class LDrawCandidateTests(unittest.TestCase):
    def test_role_classifier_pins_the_primitive_digest(self) -> None:
        ...
        drifted = role_classifier(lambda _: "sha256:0")
    ...
    def test_column_candidate_tiles_a_box_exactly(self) -> None:
        ...
        # The plate spans z -10..10, which straddles the 4 LDU column grid, so
        # the union is the tiled 24 LDU rather than the material 20. That is the
        # over-claim the score reports, and it is why the column size is swept.
    ...
    def test_a_wall_on_a_column_boundary_opens_no_column_outside_the_part(self) -> None:
        ...
        self.assertEqual(min(cell[0] for cell in cells), -5)
        self.assertEqual(max(cell[0] for cell in cells), 4)
        self.assertEqual(min(cell[1] for cell in cells), -3)
        self.assertEqual(max(cell[1] for cell in cells), 2)
    ...
    def test_a_zero_thickness_sheet_becomes_a_minimum_height_body(self) -> None:
        ...
        self.assertAlmostEqual(
            candidate.bodies[0].maximum[1] - candidate.bodies[0].minimum[1], 0.001
        )
