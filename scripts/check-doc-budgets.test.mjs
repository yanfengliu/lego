...
describe("evaluateDevlogBudget", () => {
  ...
  it("passes when every entry is within the character limit", () => {
    const text = [
      "# Devlog Summary",
      "",
      "Intro paragraph, not an entry.",
      line("2026-09-23", DEVLOG_LINE_LIMIT),
    ].join("\n");
    expect(evaluateDevlogBudget(text)).toEqual([]);
  });

  it("goes red for an entry over the character limit, naming the line, size, and limit", () => {
    const overLong = line("2026-09-23", DEVLOG_LINE_LIMIT + 1);
  ...
  it("does not grandfather an over-limit entry by date — no date is exempt any more", () => {
    // Before 2026-09-23 this exact case (a line dated well before that day)
    // was exempted by DEVLOG_BASELINE_DATE. That exemption is gone: an old
    // date no longer earns a pass, so every line is measured the same way.
    const text = ["# Devlog Summary", "", "Intro.", line("2026-01-01", 4_000)].join("\n");
    expect(evaluateDevlogBudget(text)).toHaveLength(1);
  });

  it("checks an entry line even when it carries no leading date", () => {
    const noDate = `- **no date here** ${"x".repeat(DEVLOG_LINE_LIMIT)}`;
    const text = ["# Devlog Summary", "", "Intro.", noDate].join("\n");
    expect(evaluateDevlogBudget(text)).toHaveLength(1);
  });
  ...
  it("respects a caller-supplied limit", () => {
    ...
    expect(evaluateDevlogBudget(text, { lineLimit: 20 })).toHaveLength(1);
    expect(evaluateDevlogBudget(text, { lineLimit: 40 })).toEqual([]);
  });
});
...
describe("main, against temp copies (never the real docs)", () => {
  ...
  const withinBudgetDevlog = ["# Devlog Summary", "", "Intro.", "- [2026-09-23] fine"].join("\n");

  it("goes red end-to-end when every real budget is exceeded via a temp copy", () => {
    const overLongLine = `- [2020-01-01] ${"x".repeat(DEVLOG_LINE_LIMIT)}`;
