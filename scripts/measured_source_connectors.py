...
def source_connectors_for(
    ...
    """Admit only the exact 4519 axle and 32064a axle-hole projections."""
    ...
    if design_id in ("32064", "32064a"):
        ...
        source_facts = (
            {key: source.get(key) for key in EXACT_32064A_SOURCE_FACTS}
        ...
        if (
            ...
            or eligible
            or axles
            or source_paths != ["p/axlehol5.dat"]
            or projected
            != [("axleHole", "female", [0.0, 10.0, 0.0], [0.0, 0.0, 1.0])]
            or source_facts != EXACT_32064A_SOURCE_FACTS
            or list(shadow_files) != EXACT_32064A_SHADOW_FILES
        ):
            raise ValueError(
                f"Part {design_id} requires exactly one p/axlehol5.dat A6x1 YOnly "
                "declaration composed to midpoint [0, 10, 0] with +Z normal and only "
                f"{EXACT_32064A_SHADOW_FILES} in its shadow closure; measured "
