...
export function assertFrozenLegacyDocumentProjectionV2(input: {
  ...
  if (JSON.stringify(input.document.truth) !== JSON.stringify(LEGACY_TRUTH_V2)) {
    throw new TypeError(
      "Legacy diagnostic document truth is not exact builtin.basic-parts/13 validator generation /2.",
    );
  }
  const full = validateBrickDocument(input.document);
  ...
  for (const report of input.reports) {
    ...
    const validation = validateBrickDocument(prefix);
    const prefixSequence = deriveBuildSequence(prefix);
