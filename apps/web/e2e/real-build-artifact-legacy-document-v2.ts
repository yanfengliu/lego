...
const CURRENT_RUNTIME_TRUTH_V32 = Object.freeze({
  schemaVersion: "lego.truth-snapshot/1",
  catalog: {
    id: "builtin.basic-parts",
...
/**
 * Exact `/32` live projection over the retained 85-row predecessor roster.
 ...
 * separately retained `/26` migration projection. `/32` does not move it: 41682,
 * the one part `/32` changes, is not in the predecessor roster. `/31` moved it only through
 * the nominal-stud-tube/1 profile that 2450 and 79491 gained: removing it gives
 * back the `/30` value sha256:2e127817966394853691a61fa98477fccf10e6119cfd4a8e7c8370499a19282b.
 * `/30` moved it only through 15573's centre seat: restoring 15573's `/29`
 * connectors and collision gives back the `/29` value
 * sha256:3366a2aba6de9918911d4f0bb02c82fed12a4cecd1dc9ee17710756b5a853bab.
 */
const CURRENT_RUNTIME_PREDECESSOR_SEMANTICS_HASH_V32 =
  "sha256:9cb9cbd4473e9e63d4a40c267a56638cddd814d228a78725bc0c089cb3fb2790";
const CURRENT_RUNTIME_PROFILED_PREDECESSOR_IDS_V32 = Object.freeze([
...
export function createFrozenLegacyAdditiveCatalogBasisV26(): CatalogCompatibilityBasisV26 {
  ...
  if (
    JSON.stringify(active.truth) !== JSON.stringify(CURRENT_RUNTIME_TRUTH_V32) ||
    runtimeAddedPartCounts.some((count) => count !== 1) ||
    activePartIds.length !== 106 ||
    v26PartIds.length !== 98
  ) {
    throw new TypeError(
      "Legacy diagnostic validation requires the exact reviewed /32 runtime bridge over frozen builtin.basic-parts/26.",
    );
  }
  return {
...
function currentRuntimeCompatibilityProjectionV32(document: BrickDocumentV1): BrickDocumentV1 {
  const active = createEmptyBrickDocument({
...
export function assertFrozenLegacyAdditiveCatalogV2(
  ...
  if (active.validatorSemanticsHash !== CURRENT_RUNTIME_PREDECESSOR_SEMANTICS_HASH_V32) {
    throw new TypeError(
      "Legacy diagnostic validation requires the exact reviewed /32 predecessor-semantic projection; an existing catalog interpretation moved.",
    );
  }
  ...
  if (
    ...
    JSON.stringify(CURRENT_RUNTIME_PROFILED_PREDECESSOR_IDS_V32)
  ) {
    throw new TypeError(
      `Legacy diagnostic validation requires the exact reviewed /32 predecessor profile roster; received ${profiledPredecessorIds.join(", ")}.`,
    );
  }
...
export function validateFrozenLegacyBrickDocumentV2(document: BrickDocumentV1): ValidationReportV1 {
  ...
  const projected = currentRuntimeCompatibilityProjectionV32(document);
  const validation = validateBrickDocument(projected);
  if (
    validation.truthSnapshotHash !== canonicalDigest(CURRENT_RUNTIME_TRUTH_V32) ||
  ...
  if (validation.validatorSetHash !== CURRENT_RUNTIME_TRUTH_V32.validatorSet.hash) {
    throw new TypeError(
...
export function assertFrozenLegacyDocumentProjectionV2(input: {
  ...
  const sequence = deriveBuildSequence(currentRuntimeCompatibilityProjectionV32(input.document));
  ...
  for (const report of input.reports) {
    ...
    const prefixSequence = deriveBuildSequence(currentRuntimeCompatibilityProjectionV32(prefix));
