...
// The /13 roster holds every part the two /29 -> /30 rows name (61 LDraw
// frames and 15573), so the live report carries both unfiltered.
...
// /32 changes only 41682, which enters the /13 roster as an added part, so it adds no row.
...
describe("reviewed frozen-/26 additive legacy operation projection", () => {
  it("uses active truth only transiently and restores the exact /13 source truth", () => {
    ...
    expect(
      VERSION_30_INTERPRETATION_CHANGES.map(
        ({ affectedCatalogPartIds }) => affectedCatalogPartIds.length,
      ),
    ).toEqual([61, 1]);
    ...
    const reconstructed = applyReviewedAdditiveLegacyBuildOperations(
      ...
      transaction.operations,
      {
        ...
        applyBuildOperations: (document, operations) => {
          ...
          if (document.truth.catalog.version !== "builtin.basic-parts/32") {
  ...
  it("permits an unconnected changed /13 part but refuses its changed endpoint", () => {
    ...
    expect(() => projectExactCurrentMigrationToFrozenV26(connected, refused)).toThrow(
      /exact reviewed \/13 to \/32 runtime migration bridge/u,
    );
  });
  ...
  it("refuses runtime migration drift before projecting the frozen /26 boundary", () => {
    ...
    ).toThrow(/exact reviewed \/13 to \/32 runtime migration bridge/u);
