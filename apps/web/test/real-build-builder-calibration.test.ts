 ...
 *
 * These three inputs used to resolve against the working directory, so every
 * one of them was "absent" whenever vitest ran from anywhere but the repository
 * root and the recompute case below silently skipped: `cd apps/web && npx
 * vitest run` reported "1 passed" while the strongest assertion this repository
 * makes about Builder frame truth did nothing at all. A green run from a
 * subdirectory was indistinguishable from a green run that had checked the
 * calibration. `real-build-bootstrap-source.test.ts` in this same directory
 * already anchors its root this way, and `real-build-bootstrap-source.ts` says
 * why: a run started from anywhere has to resolve the checkout the artifacts
 * actually belong to.
 */
const REPOSITORY_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..", "..");
const OFFICIAL_MODEL_FILE = "output/official-model/vx1087034_21066_a.xml";
const GEOMETRY_BUNDLE_FILE = "output/real-build/builder-shell-geometry.bin";
const CALIBRATION_FILE = "output/real-build/builder-canonical-calibration.json";
const REGENERATE_INPUTS =
  "LEGO_REAL_BUILD_REGENERATE_INPUTS=1 npx playwright test apps/web/e2e/real-build-inputs.spec.ts";

/**
 * Reads a generated real-build input, or fails naming the file that is absent.
 *
 * These are inputs a working checkout is expected to have and one documented
 * command regenerates, not optional fixtures, so absence is a failure rather
 * than a skip. A skip cannot be told apart from a pass by an exit code, and
 * that is this repository's recurring failure mode — two gates were found
 * measuring nothing on 2026-08-06, and a ledger assertion spent 53 minutes'
 * worth of commits comparing a constant against itself. Nothing about a fresh
 * clone is worth buying back at the price of a gate that can quietly stop
 * running: a checkout without these files cannot make the claim this case
 * exists to make, and should say so.
 *
 * Since the 2026-09-22 reset the cases that read these inputs run only under
 * `LEGO_RUN_EVIDENCE=1` (`scripts/run-evidence-gate.mjs`): the retained report
 * belongs to the retired first-50 campaign and no longer matches the catalog.
 * The default gate reports them as skipped with that reason in their names.
 * Opted in, an absent input still fails here by name rather than skipping.
 */
function readRequiredRealBuildInput(repositoryRelativePath: string): Buffer {
  ...
  if (!existsSync(path)) {
    throw new Error(
      `Required real-build input ${repositoryRelativePath} is missing: nothing at ${path} ` +
        ...
        `real-build inputs with \`${REGENERATE_INPUTS}\`. These cases are not optional — they ` +
        `are where the retained Builder frame report is re-derived from the pinned geometry ` +
        `bundle and the live catalog and byte-compared, so an absent input is failed by name ` +
        `rather than skipped.`,
    );
  }
...
const IN_STEP = "pinned to the current catalog version";
 ...
 *
 * A report pinned to a superseded version claims agreement with a catalog that
 * no longer exists. That is failed here by name rather than announced in a
 * warning nobody reads. Staleness used to also silence the recompute case
 * below; that case now always runs and checks this first, so a bump still reads
 * as one sentence instead of a fifteen-frame object diff.
 */
function retainedCalibrationCatalogState(calibrationBytes: Buffer): string {
  const retained = JSON.parse(calibrationBytes.toString("utf8")) as {
  ...
  if (versions.length > 0 && versions.every((version) => version === BUILTIN_CATALOG_VERSION)) {
    return IN_STEP;
  }
  ...
  return (
    `Retained ${CALIBRATION_FILE} is pinned to catalog ` +
    `${versions.join(", ") || "no designFrames at all"} and this build is ` +
    `${BUILTIN_CATALOG_VERSION}. Every part definition carries the catalog version in its ` +
    `provenance, so a bump invalidates the report and the real build then rejects with ` +
    `builder-calibration-invalid. Regenerate it with \`${REGENERATE_INPUTS}\`. Do not edit the ` +
    `artifact into agreement, and do not delete it — this case fails on an absent report too.`
  );
}
...
const itWithEvidence = itWithRunEvidence(
  "reads output/real-build/builder-canonical-calibration.json and builder-shell-geometry.bin, retained by the retired first-50 campaign and no longer at the current catalog version",
);

describe("Builder canonical calibration v8", () => {
  itWithEvidence("keeps a retained calibration report in step with the catalog it claims", () => {
  ...
  itWithEvidence("rejects a structured clone of the opaque 2453 source row", () => {
