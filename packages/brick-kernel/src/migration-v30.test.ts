 ...
 * catalog, so a later truth change shows up here and must get its own row, as
 * /32's 41682 recess seats do.
 */
...
describe("builtin.basic-parts/30 migration", () => {
  it("pins the complete /30 truth and the snapshot that names it", () => {
    ...
    expect(
      REVIEWED_HISTORICAL_TRUTH_SNAPSHOTS.find(({ truthHash }) => truthHash === V30_TRUTH_HASH),
    ).toEqual({
  ...
  it("pins the complete /30 connector authority that /31 reinterprets", () => {
    expect(REVIEWED_HISTORICAL_CONNECTION_SEMANTICS_BY_TRUTH_HASH[V30_TRUTH_HASH]).toEqual({
      ...
      // /32 adds 41682's two recess seats, which this source does not have either.
      endpointDeltas: plusV32(sortedEndpointDeltas(EXPECTED_V31_STUD_PROFILE_CHANGES), 30),
      pairDeltas: [],
  ...
  it("carries each /31 stud delta in every source truth that has its part, and only those", () => {
    ...
    for (const { truthHash } of REVIEWED_HISTORICAL_TRUTH_SNAPSHOTS) {
      ...
      // A /31 source already declares the seven profiles, so no stud of theirs moves for it.
      const held = EXPECTED_V31_STUD_PROFILE_CHANGES.filter(
        ({ partId }) =>
  ...
  it("carries a /30 document to /31 and names the stud-profile reinterpretation", () => {
    ...
    // Only the catalog label moves: /31 and /32 keep the /30 connector
    // taxonomy, collision model, transform policy and validator set versions.
  ...
  it(
    "names exactly the parts whose interpretation moved since /30 source commit c6356f7",
    async () => {
      ...
      // Collision moved for the seven (/31) and 41682's recess seats for 41682 alone
      // (/32); no geometry or frame did.
      ...
      for (const field of fields) {
        if (field === "collision" || field === "connectors") continue;
