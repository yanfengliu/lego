...
import {
  ...
  carriedPlusV31,
  expectedV31InterpretationChanges,
  plusV31,
  ...
  it("carries 15573's three /30 deltas in every source truth that has 15573, and only those", () => {
    for (const { catalogVersion, truthHash } of REVIEWED_HISTORICAL_TRUTH_SNAPSHOTS) {
      ...
      expect(
        ...
        truthHash,
      ).toEqual(hasJumperChange ? EXPECTED_JUMPER_1X2_CENTRE_SEAT_CHANGES : []);
      // Every such row carries saved edges on the two grid clutches; no other row carries any.
      expect(
        row?.carriedEndpointDeltas?.filter(({ partId }) => partId === JUMPER) ?? [],
        truthHash,
      ).toEqual(hasJumperChange ? EXPECTED_JUMPER_1X2_CARRIED_ENDPOINT_DELTAS : []);
    }
  });
