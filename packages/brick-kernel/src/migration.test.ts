...
import { version30InterpretationChangesForSourceCatalogVersion } from "./migration-v30.test-support.ts";
...
const VERSION_27_ADDITIVE_PART_IDS = [
...
const VERSION_28_ADDITIVE_PART_IDS = [
...
const POST_V13_ADDITIVE_PART_IDS = [
...
describe("migrateDocumentTruth", () => {
  ...
  it("names every in-place visual reinterpretation when a /11 document advances", () => {
    ...
    expect(report.migrated).toBe(true);
    expect(report.addedCatalogPartIds).toEqual(POST_V13_ADDITIVE_PART_IDS);
    expect(report.catalogInterpretationChanges).toEqual([
      ...
      ...version30InterpretationChangesForSourceCatalogVersion(11),
    ]);
  ...
  it("reports 24 normal and geometry plus 12 bounds reinterpretations at /13", () => {
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      ...VERSION_13_INTERPRETATION_CHANGES,
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(12),
    ]);
    expect(document.parts).toEqual(savedAtTwelve.parts);
  });

  it("carries /13 forward and reports all nineteen complete additive definitions", () => {
    const savedAtThirteen = historicalDocument({
      ...
      allowedCatalogPartCount: PART_DEFINITIONS.length - 19,
    });
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(13),
    ]);
    expectReviewedCurrentTruthChanges(report, "builtin.basic-parts/13");
    expect(document.parts).toEqual(savedAtThirteen.parts);
  });

  it("carries /14 forward and reports the eighteen complete additive definitions", () => {
    const savedAtFourteen = historicalDocument({
      ...
      allowedCatalogPartCount: PART_DEFINITIONS.length - 18,
    });
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(14),
    ]);
    expectReviewedCurrentTruthChanges(report, "builtin.basic-parts/14");
    expect(document.parts).toEqual(savedAtFourteen.parts);
  });

  it("carries /15 forward and reports the seventeen complete additive definitions", () => {
    const savedAtFifteen = historicalDocument({
      ...
      allowedCatalogPartCount: PART_DEFINITIONS.length - 17,
    });
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(15),
    ]);
    expectReviewedCurrentTruthChanges(report, "builtin.basic-parts/15");
    expect(document.parts).toEqual(savedAtFifteen.parts);
  });

  it("carries /16 forward and reports the sixteen complete additive definitions", () => {
    const savedAtSixteen = historicalDocument({
      ...
      allowedCatalogPartCount: PART_DEFINITIONS.length - 16,
    });
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(16),
    ]);
    expectReviewedCurrentTruthChanges(report, "builtin.basic-parts/16");
    expect(document.parts).toEqual(savedAtSixteen.parts);
  });

  it("carries /17 forward and reports the next fifteen complete measured definitions", () => {
    const savedAtSeventeen = historicalDocument({
      ...
      allowedCatalogPartCount: PART_DEFINITIONS.length - 15,
    });
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(17),
    ]);
    expectReviewedCurrentTruthChanges(
      report,
  ...
  it("carries /18 forward and reports the next fourteen complete measured definitions", () => {
    const savedAtEighteen = historicalDocument({
      id: "eighteen",
      name: "Saved at /18",
      catalogVersion: "builtin.basic-parts/18",
      catalogHash: "sha256:9e5b17400da2280cb46f97ff69aec063c07a5474f3153a5826b3b88043c36ff6",
      connectorHash: "sha256:c852c97a2cc665a624e78c656ffcba43b6792a1e04ba91bc4059b3f0558baac0",
      collisionVersion: "rectilinear-stud-clearance/3",
      collisionHash: "sha256:f09f42d55dbe7e3bf48b2681499f4475dca3ee686caeac1f417d78dee955bddd",
      transformHash: "sha256:e557ade41c1f739dbf41c59033bd59adab18d87e444321567db4c0d7fa44a41b",
      validatorVersion: "lego.kernel-validators/3",
      validatorHash: "sha256:fb0676931eb66a0096f393794d0be1297227811a77b986c0a1d05847ee3127d4",
      allowedCatalogPartCount: PART_DEFINITIONS.length - 14,
    });
    expect(canonicalDigest(savedAtEighteen.truth)).toBe(
      "sha256:8172cc4f993b46bb9fa8f782bb2b295c516e95c16f2d6861e4a18219ef2e1b20",
    );
    const { document, report } = migrateDocumentTruth(savedAtEighteen);
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(18),
    ]);
    expectReviewedCurrentTruthChanges(
      report,
  ...
  it("carries /19 forward and reports the next thirteen complete measured definitions", () => {
    const savedAtNineteen = historicalDocument({
      id: "nineteen",
      name: "Saved at /19",
      catalogVersion: "builtin.basic-parts/19",
      catalogHash: "sha256:90eae14b0755f6c2b9d5515f4e5db53966d938b5d9867ee1aed90b09ea247016",
      connectorHash: "sha256:03ccce5b7d3ad14c6b9c9749abb3a806139ade4f786919d54e989ca1a14c6750",
      collisionVersion: "rectilinear-stud-clearance/3",
      collisionHash: "sha256:911806345cb509dad7c3b0c923f8d87364c66e71627ffec9bd934b4df344f3fd",
      transformHash: "sha256:d2888660cff26c2f5665e76c02fecc532b3a04aada1810695495230eb5f664d9",
      validatorVersion: "lego.kernel-validators/3",
      validatorHash: "sha256:fb0676931eb66a0096f393794d0be1297227811a77b986c0a1d05847ee3127d4",
      allowedCatalogPartCount: PART_DEFINITIONS.length - 13,
    });
    expect(canonicalDigest(savedAtNineteen.truth)).toBe(
      "sha256:e34fcc8ac627f0dcfdb1d779246a723101d765f931830a4c06514d9daff75c26",
    );
    const { document, report } = migrateDocumentTruth(savedAtNineteen);
    ...
    expect(report.catalogInterpretationChanges).toEqual([
      VERSION_29_CONNECTOR_INTERPRETATION_CHANGE,
      ...version30InterpretationChangesForSourceCatalogVersion(19),
    ]);
    expectReviewedCurrentTruthChanges(
      report,
  ...
  it.each([
    ...
    },
    {
      ...
      validatorVersion: "lego.kernel-validators/3",
      validatorHash: "sha256:fb0676931eb66a0096f393794d0be1297227811a77b986c0a1d05847ee3127d4",
      futurePartId: "builtin:bracket-2x2-1x2-vertical-studs",
    },
    {
      catalogVersion: "builtin.basic-parts/19",
      catalogHash: "sha256:90eae14b0755f6c2b9d5515f4e5db53966d938b5d9867ee1aed90b09ea247016",
      ...
      validatorVersion: "lego.kernel-validators/3",
      validatorHash: "sha256:fb0676931eb66a0096f393794d0be1297227811a77b986c0a1d05847ee3127d4",
      futurePartId: "builtin:brick-1x2-grille",
    },
  ] as const)(
  ...
  it("carries a /6 document forward and names the twenty-seven parts it gained", () => {
  ...
  it("carries a /7 document forward and names the twenty-two parts it gained", () => {
    const document = historicalDocument({
      id: "seven",
      name: "Saved at /7",
