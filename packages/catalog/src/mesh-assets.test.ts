...
describe("preloaded mesh asset resolution", () => {
  it("keeps the exact promotions in place and appends the new source mesh", () => {
    // Sixteen existing rows render exact source meshes at /13; /14 through /25
    ...
    expect(meshParts).toHaveLength(36);
    expect(fullyMeasuredParts).toHaveLength(20);
    expect(
      ...
      createHash("sha256")
        .update(
          JSON.stringify(legacyParts)
            .replaceAll("builtin.basic-parts/25", "builtin.basic-parts/15")
