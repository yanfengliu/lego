import { STUD_HEIGHT_LDU, STUD_PITCH_LDU, STUD_RADIUS_LDU } from "./constants.ts";
import type { LduBounds, PartTubeFeature } from "./types.ts";
...
interface ConnectorBackingGeometryInput {
...
/**
 * The questions `buildConnectorFeatures` puts to a part's body before it admits
 * a connector: whether a footprint cell is solid, whether a face backs a stud,
 * whether the modelled cavity holds one, and how much of a stud an arc edge
 * still backs.
 */
export const connectorBackingGeometry = ({
  blueprint,
  topY,
  bottomY,
  bodyBoxesLdu,
  bodyTubes,
}: ConnectorBackingGeometryInput) => {
  const { bodyWedge, bodyArc } = blueprint;
   ...
   * exactly the volume the `tubeSeat` allowance in `buildConnectorFeatures`
  ...
  return { cellIsSolid, faceHoldsStud, cavityHoldsStud, partialOverhangBackingFraction };
};
