...
Then remeasure, score, canonicalize, and compare the generated catalog outputs. The generator owns 16 tracked outputs: the mesh registry, seven bounded measured mesh shards A through G, one render-only mesh shard, the measured blueprint root plus bounded D through G shards, one render-only blueprint shard, and the bundled-source table. The render-only generated type cannot carry connector, allowance, or collision truth. `--check` runs the workspace-pinned Prettier and refuses any byte drift, while report schema `/6` records 23 measured-pipeline rows and 12 dedicated render-only-pipeline rows, binds the exact pilot and Builder-frame inputs, records optional source-bounded connector rows such as the three discrete `4519` axle seats, the single transverse `32064` axle-hole seat, and the nine-stud/nine-clutch `11212` lattice, and retains the optional validated-connection stud profile without weakening ordinary collision. Four of those measured-pipeline rows are final catalog render-only promotions whose earlier physical authority is overlaid by the catalog factory, so the catalog result is nineteen fully measured definitions plus sixteen render-only promotions rather than the report's generation-path 23/12 split. The `32064` axle-hole row remains a discrete structural endpoint with no continuous axial authority. The `11212` row comes only from exact `p/stud.dat`, `p/stud4.dat`, and `parts/11212.dat` shadow routes; exact source-radius cylinders remain ordinary collision, while nominal radius 6 applies only to exact validated edges. [Part-model catalog truth](../design/part-model.md#current-catalog) owns the current `/24` counts, mesh deltas, and migration interpretation.
...
The current `/24` instrument control is the ignored 203,106-byte schema-`/6` check report at `output/real-build/set-6651557-measured-part-emission-v24-check.json`; it reproduces all 16 generated files with `written:false` at `sha256:7aeccd85ad90106af16eb5c152a93162e3cb22be1793731e75673d1d02f45719`. Its `sampleSpacingLdu` field remains the backward-compatible requested-spacing alias; the additive requested, maximum-effective, 256-subdivision-limit, and cap-limited-triangle fields disclose the grid actually sampled without changing the pinned older report envelopes. That report is generator evidence, not independent source, catalog, physical, or build authority.
...
The separate tracked ledger/current-catalog identity check now measures 18 of the immutable 121 required leaf design IDs at `sha256:71648d320171d583cbff1ad4768e2e9e192747db48af67cf529e7c3649021847`. That inventory intersection neither regenerates the ignored `catalog-coverage.json` nor establishes a continuous printed-step prefix or build frontier; `11212` has no trusted printed-crop or placement authority, and the stale step-39 pair-judged and step-101 unreviewed `32064` locators remain counterevidence only. The action ledger remains through step 25, with the step-26 `28802` versus Builder `10201` contradiction unresolved.
