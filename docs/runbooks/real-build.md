...
The separate exact-five capture command exercises only those five rows and writes or publishes no retained artifact yet: its current spec reconstructs the bounded browser wire in Node and closes exact high/work RGBA, H/P/D/W, pairwise facts, and lossless PNGs, then discards the authority-free capture because publication and execution-identity binding are deliberately not implemented. The real-booklet opt-in has not been run for this unit.
...
The retained-build command now creates the exact canonical empty document, reserves all eight step-0 D4 roots against the run's cumulative branch budget, retains them in report 1 under browser-output `/3`, and intentionally refuses printed step 1 before any placement callback because scoring is not yet carried through those immutable lineages. The `/3` reader exact-binds every action and evidence digest to its prepared panel, bounds terminal `documentJson` by encoded size, nesting, structural nodes, steps and parts, parses it once, and binds the terminal document to accepted hash and part continuity, global validity, ordered accepted `BuildStep` ID/index/semantic-name/owned-part facts, canonical root ownership, metadata and manual provenance. The root refusal must therefore end at the exact empty document, and every later requested row must name step 1 while retaining exact zero elapsed, raster and candidate work with no validation, render, camera or document drift. Fixed-ledger omissions and `multi-build-copy` runner call sites now pass through `executeRealBuildFixedActionWithPhysicalAuthority` before their executor; every current decision returns typed `fixed-ledger-frame-unresolved` because no trusted producer can mint the required opaque physical-frame authority. Running the command therefore neither repairs nor supersedes the retained mirrored diagnostic.
...
For that case, `diagnostic-prefix.json` retains the exact `BrickDocumentV1` candidate separately. Current score `/5` and artifact-manifest `/4` bind the diagnostic summary under the generation-3 contract and browser boundary; an eligible downstream verifier independently finalizes retained browser-output `/3`, reproduces the diagnostic document bytes, validates document/hash/part/contiguous-step facts, and requires the file, score, and manifest to agree. Metadata-only and input-rejection boundaries cannot enter this verifier until they retain typed digest-bound rejection evidence, and no production artifact currently exercises the current tuple. The frozen legacy score `/4` and artifact-manifest `/3` encode the corresponding historical summary under contract/browser `/2` and are accepted only by the explicit inspection-only verifier. Either closure proves which diagnostic bytes survived under its own schema generation, not that they equal the official model.
...
Retained run `2026-08-12T10-28-52-560Z-34694c87c62e-26128a38-4d9e-4294-9091-6aae9b3ca367` lives under `output/direct-origin-k-production/runs/` as immutable legacy inspection-only evidence. It uses artifact-manifest `/3`, run-contract `/2`, browser-output `/2`, result/score `/4`, and replay-closure `/3`; the exact legacy inspector verifies that tuple without exposing current publication or pointer authority. It uses generation-local frozen browser, farther, capture, atomic report, completion-failure and identity predicates rather than current finalizer helpers, and separately replays the pinned semantic and per-step prefix validation under the historical truth generation. Its historical `current.json` binds replay digest `sha256:1c27df8a95c655f7508436489e8e31f486f806c7a5382df76d53e0a80801a66c` and artifact-manifest digest `sha256:4dc5ce021e03ba0bc86667a64a1948f9ad54d2d80f95eb887b559a593c7036aa`. The measured score file remains `sha256:cbdf5b5502448011356b7fdb15f655734e97021853a45f41d64f63fd3f9e042e`, and `diagnostic-prefix.json` remains `sha256:2edf84fbf1eab57e86cd2670f9bdb5e60a7ac33dbda454f22d9c9a85cbf8b70f`.
