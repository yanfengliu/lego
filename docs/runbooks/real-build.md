...
```powershell
npx.cmd vitest run apps/web/test/real-build-builder-prefix-contract.test.ts apps/web/test/real-build-builder-frame-selection.test.ts apps/web/test/real-build-builder-calibration.test.ts
python -B scripts/builder_calibration_scripts_test.py
python -B scripts/builder_ldraw_frame_test.py
```

The current source registry has 43 revisions covering 197 of 320 physical identities. Exact local/world composition must yield 182 proper-upright usable rows, the pinned 15-row non-upright diagnostic roster, and 138 total canonical-upright refusals. The ignored geometry bundle must be 1,820,412 bytes at `sha256:7e91e1402f2ab609fee6e502336f86ee74fb3a94d970e9b0b75acf07f925a76f`; the 184-file official closure must be `sha256:72ca520b68934fdaa384e9bbc961090538f0b4ee1269773675db1adcf3cc7fdd`; calibration `/8` must be 54,993 bytes at `sha256:40807a2329f43321c4af683096465de57f41be41299bbd5b7dea8354e28fe8e9`. Only Builder families 0/1 may anchor a catalog stud and family 15 a catalog clutch; an unmapped authored lattice still blocks surface-only fallback. The exact 320-row action/source roster is 43,528 bytes at `sha256:559d808d9fe3980f1f3be73718db2305b1e341cd4d40b5811e2e4d2c3d507d6a`; the closed-field 76,854-byte proper-world diagnostic is `sha256:f01f40fe744f0f704411e470852339eb5d285eacfea73680052421778a7445a9` and refuses caller-remapped provenance while dropping uncommitted caller fields. Ten revision classes are diagnostic holdouts with null executed evidence and no claimed failure mechanism. This census grants no source execution, official world placement, replay, mutation, acceptance, or completion authority.
...
The ignored artifact must be 500,895 bytes at `sha256:7b76ef27bbad99f9014fd6543b0fdb47c1fdcfd789d0d9629e16df54a4889da7`. It independently reconciles 1,465 XML Brick rows with 1,465 top-level LDraw rows and 1,469 flattened leaves, then retains all 320 prefix action rows: 309 exact catalog-world proposals and eleven quarantines. The quarantines are two `10201.dat` versus catalog `28802.dat`, seven official `3245b.dat` versus catalog `3245c.dat`, and one each of exact `41769.dat`/`41770.dat` versus unpinned catalog `41769a.dat`/`41770a.dat` frames. Three step-45 `4519` origins remain exactly half-LDU at z `-96.5`; do not round or infer document legality. The artifact is a local diagnostic proposal only and grants no source execution, physical frame, action, placement, document mutation, replay, acceptance, or completion authority.
...
The exact-frame artifact must be 98,383 bytes at `sha256:16a76321d1cbd8bb1308dff9c58ee61507f67c0139b17aa50a5d8415c789e2f2`: 62 projectable aliases comprising 27 mesh-backed frames, 30 existing geometry-proved parametric frames, and five archive-derived frames for `15573;L`, `3003;S`, `3024;N`, `3069;Q`, and `3622;J`. It must exclude all four quarantined alias classes and their eleven occurrences, bind the exact 144,722,356-byte official archive at `sha256:6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae`, and retain frame-table digest `sha256:66d69363a3d7d42127f70ccb42e0a7c764efceba4d96386b9751abd9f5dde973`. The world artifact must be 408,269 bytes at `sha256:63d5872f92d208755dae34ecab7bad31c23b9b4cdcfb4165bbbc3159d281156e`, retain all 320 actual source references, reconcile 298 direct plus eleven real `MultiBuild` rows, quarantine eleven direct rows with null frames and worlds, and reproduce occurrence/world commitments `sha256:63ab9b3bce415598f3a7b7fa0b407f29502df86f6a761fb541191fef28ca7b84` and `sha256:da7932eeaeb115e9db38ac7ef01f9b4142df46fd1d3b2eb0a484e09aafa989fe`. The reconciler must derive actual/source/master copy provenance from its third opaque action-preparation input; no static occurrence table may author `MultiBuild` kind. Its first-eight connector-only audit must produce one component after 19,008 pair comparisons over 214 catalog connectors, but that topology is an instrument check rather than collision, document-legality, or attachment authority. The official parser reproduces 561 physical phases and 132 separately ordered completion events; the 7,292-byte structural artifact at `sha256:cdb563b81f89aa9110cc70cc392f7415616aeb3e534ee907547879f8ec82c03e` digest-binds those parser outputs while publishing event 8, the zero-piece printed-step-44 `sub-build-complete` return boundary over its 23 exact child members after phase 71 and before phase 72. Its separately retained unauthenticated attachment interpretation does not gate or label the official event, and no connection edge is inferred. All three artifacts are ignored evidence; parsed lookalikes, forged tokens, same-design swaps, `MultiBuild` source substitution, step 51, suffix aliases, and incomplete transforms must refuse. They grant no action, placement, mutation, replay, acceptance, or completion authority. Each generator compares its in-memory reproduction with the reviewed pin before writing, so drift refuses without overwriting retained evidence.
...
The separate tracked ledger/current-catalog identity check measured 19 of the immutable 121 required leaf design IDs under historical `/25` at `sha256:7859f65b300f4e8a2b6dae85cffedbcd71e5f54fcffd8f688089c3353c549f28`; current `/28` remains 24 at `sha256:1065c3a3d04787974aa43c8ede60c1937930884bdf297488d3731dc8c99ea591`: exact suffix admissions `3245c` and `2453b` do not normalize bare or cross-suffix IDs and therefore do not themselves add the Builder `3245;M` or `2453;I` rows. That inventory intersection establishes neither a continuous printed-step prefix nor a build frontier. The separate exact `2453;I` proof now binds its solid-`2453b` identity and local-part-frame route, and the diagnostic frame registry consumes only its opaque item-`6595205` route; item `4210690`, suffix aliases, and `3245;M` remain unresolved. Current semantic coverage `/4` is 588,467 bytes at `sha256:a12d5744f3f4417628e53227aaa4c35d9aee0eba5fdce7b865087e6f97dfbfad` and measures all 49 part-bearing rows / 320 pieces catalog-placeable; action preparation `/1` orders all 320 identities but remains refused by production action consumers. No official world placement, trusted action ledger, replay, source execution, acceptance, or step-frontier authority follows, and the step-26 `28802` versus Builder `10201` contradiction remains unresolved.

In this chain, `LEGO_REAL_BUILD_LAST_STEP` is mandatory and selects the exact identity/action prefix for both a requested coverage rebuild and stage-4 ledger compilation. It does not truncate the callout manifest or source index: those remain dense through all 359 printed steps. Coverage compiled for one prefix is deliberately not reusable as a longer or shorter prefix, and the publisher refuses a mismatch instead of silently widening work.
