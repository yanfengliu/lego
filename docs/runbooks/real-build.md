...
`document.json`, `score.json`'s canonical `structuralHash`, and `finalParts` describe only a finalizer-approved official-target document. When searched transforms differ from the raw official ledger and the run retains no independently proved proper world-frame mapping, a structurally valid candidate must leave those outputs absent, null, and zero. Reflection is not a permitted frame reconciliation.
