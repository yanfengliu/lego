...
Then remeasure, score, canonicalize, and compare the generated catalog outputs. The generator owns 16 tracked outputs: the mesh registry, seven bounded measured mesh shards A through G, one render-only mesh shard, the measured blueprint root plus bounded D through G shards, one render-only blueprint shard, and the bundled-source table. The render-only generated type cannot carry connector, allowance, or collision truth. `--check` runs the workspace-pinned Prettier and refuses any byte drift, while report schema `/6` records 22 measured-pipeline rows and 12 dedicated render-only-pipeline rows, binds the exact pilot and Builder-frame inputs, records optional source-bounded connector rows such as the three discrete `4519` axle seats and the single transverse `32064` axle-hole seat, and retains the optional validated-connection stud profile without weakening ordinary collision. Four of those measured-pipeline rows are final catalog render-only promotions whose earlier physical authority is overlaid by the catalog factory, so the catalog result is eighteen fully measured definitions plus sixteen render-only promotions rather than the report's generation-path 22/12 split. The `32064` row comes only from the exact `p/axlehol5.dat`, `p/stud2.dat`, and `parts/32064a.dat` shadow tree and remains a discrete structural endpoint: it does not establish continuous axial sliding, grip, stability, insertion access, or collision relief through the bore. [Part-model catalog truth](../design/part-model.md#current-catalog) owns the current `/23` counts, mesh deltas, and migration interpretation.
...
The current `/23` instrument control is the ignored schema-`/6` check report at `output/real-build/set-6651557-measured-part-emission-v23-check.json`; it reproduces all 16 generated files with `written:false` at `sha256:57a7a8909f2f6d9ff27e79185970a796acd70f82d6e94b4b922de8c672768d07`. Its `sampleSpacingLdu` field remains the backward-compatible requested-spacing alias; the additive requested, maximum-effective, 256-subdivision-limit, and cap-limited-triangle fields disclose the grid actually sampled without changing the pinned older report envelopes. That report is generator evidence, not independent source, catalog, physical, or build authority.
...
The current `/23` source-closure pin is `lego.real-build-source-attestation/1` over 3,598 repository files at `sha256:ca8411a777c37f97d070f095d0948fd57258a00237bacedf5292ce057621598e`, declared in `apps/web/e2e/real-build-farther-origin-source-manifest.ts`. It is the exact executable-source condition for the calibrated exception, not a fresh 14,172-row depth run, a score rebind, visual target truth, or build progress.
...
The separate tracked ledger/current-catalog identity check now measures 17 of the immutable 121 required leaf design IDs at `sha256:c5b20fa64d9d0457822de5bbecc3a66f4f1633b2004e7e23342a3c95f99c6633`. That inventory intersection neither regenerates the ignored `catalog-coverage.json` nor establishes a continuous printed-step prefix or build frontier; the stale step-39 pair-judged and step-101 unreviewed `32064` locators remain counterevidence only.
