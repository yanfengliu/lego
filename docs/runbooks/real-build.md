...
The current 400-offer enumerator is not directly quotientable: generated first-part IDs make its raw rows singleton groups. A read-only census over bytes returned by the existing witness projection measured exactly 100 proper-C4 groups of four; the projection validates then omits candidate `partIds` and `restsOnBuildPlate`, rewrites generated connection targets to witness references, and injects prepared catalog/color identity. It does not group or quotient offers, and no executable orbit gate exists. Any future quotient must retain the original raw member IDs and support data because projected witnesses alone cannot reconstruct those fields or byte-identical compiler children. Its proper-yaw action rotates both witness `positionLdu` values and adds `q` to both orientation IDs while preserving identity key, catalog ID, color, witness target, port IDs, connection kind and order. It must prove the camera mapping `t_rep = t_member - d*q (mod 360)` for member rotation `q` and determinant `d`: as-fitted subtracts `q`, x-reflected adds `q`, hand/determinant stay unchanged, and neither model reflection nor target transforms may enter. Masks, shifts, scores and ranking must also reproduce before compilation or camera work can be reduced.
