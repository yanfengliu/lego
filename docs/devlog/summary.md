...
- 2026-09-24 (G3f-3, Playwright specs onto `LEGO_RUN_EVIDENCE`): 24 of 26 booklet-gated `.spec.ts` files converted from a bare `hasSampleBooklet`/`existsSync` skip to `apps/web/e2e/run-evidence-gate.ts`'s `skipWithoutRunEvidence`; one compound case kept its onlyIf; `real-build-step7-gate3-diagnostic.spec.ts` was already unconditional-by-default and fails loudly (not skips) when opted in, so it was left alone. `run-evidence-gate-coverage.test.mjs` now also scans Playwright's spec population, read from `playwright.config.ts`'s own `testDir`/`testMatch`; `hasSampleBooklet` is a named existence token since it is always imported, never locally declared, so the scan's one-hop resolution could not otherwise reach it. `npm run test:browser` default gate: 59 passed/14 skipped (main) → 45 passed/28 skipped here (exactly the 14 converted specs, same booklet-present machine).
