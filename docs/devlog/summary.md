...
- [2026-09-24 (renderer handedness)](detailed/2026-09-24_2026-09-24.md) — LDU reached Three as (x,-y,z), a reflection: every model drew mirrored, step 31 flipped against page 35. Now the half-turn (x,-y,-z) everywhere; `lego.rendering/2`; det, chiral-pair and pick gates. Physics carried the same bug: the simulation session forwarded Rapier's raw sim-frame rotation unconverted, so a tilt rendered backwards; converted at the session boundary, with a tilt-direction gate and a strengthened picking gate.
