...
4. For engineering-loop onboarding, read the global `building-recursive-loop` skill and its canonical `fleet` build recipe.
5. For an existing engineering improvement pass, read the global `recursive-playtest` skill and the current `fleet/DIRECTIVES.md` before running it.
