...
## A classifier with fewer outcomes than the world reports a confident positive

Three of these surfaced in one afternoon, in three different files, written by two agents who each caught the other's and then found their own.

A defect-side classifier had two outcomes, `inventory-thumbnail` and `callout-crop`, and had to answer with one of them. Given a cluster whose callout and thumbnail agreed exactly — a 1.0x ratio, meaning the geometry is fine on both sides and the miss is elsewhere — it returned `callout-crop`. It invented a defect out of a tie and pointed the repair at a file that was correct.

A lookup reported an element as having no same-mould sibling. It had one. The answer came from searching a published `worst[:15]` list, so "absent from a truncated view" and "does not exist" were the same result, and the absence was reported as a fact.

A report built its miss list as "rank is None or rank > k", then built the cause block with a trailing `if rank is not None`. A truth element with no thumbnail is a miss that cannot be ablated, because there is no descriptor to ablate, so it was dropped between the two and counted nowhere. The per-repair totals would have summed to fewer misses than existed and looked complete doing it.

The shape is the same each time: a case the instrument cannot speak about is answered as though it could, and the answer is a positive rather than an absence. Nobody reads a confident wrong answer as a missing one.

Two things follow. Give "cannot tell" its own outcome, distinct from every substantive verdict and from silence — `no-sibling-to-compare` and `not-measurable-no-thumbnail` are rows, not skips. And publish a conservation check the caller can read: rows equals ablatable plus unreachable, asserted rather than assumed.

The third was latent — zero unreachable truths existed in that generation, so it had never fired, and a live test would have reported green forever on a branch real data never enters. It is driven by synthetic cases instead. A branch your data cannot reach is not covered by a passing suite.

**Anchor:** `test_agreement_on_both_sides_is_its_own_outcome` in `scripts/part_description_retrieval_test.py`; `everyMissAccountedFor` and the three synthetic drivers in `scripts/part_retrieval_ceiling_test.py`; `elementsWithNoSibling: 113` and `worstIsATruncatedView` in `output/part-retrieval-ceiling.json`. All 2026-08-06.
