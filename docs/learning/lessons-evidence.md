...
The booklet refuted it. Refitting all forty measured panels a second time as below-views produced no solution on any of them — not on the five drawn from underneath either. The reason is exact rather than a matter of tolerance: a below-view lattice at azimuth A is the same lattice as an above-view at azimuth -A, because `a(A, -e) = a(-A, e)` and `b(A, -e) = -b(-A, e)`, and negating one basis vector spans the same lattice. The fitter searches over re-basings, so it always reaches the positive-elevation twin. A `face` option on it could never change an answer.

The mistake was building a discriminator without first asking whether the two cases are distinguishable at all. They are not: the ambiguity is an identity, not a near-miss, and no amount of extra evidence handed to the *measurement* can break it. What the cue does is decide which of two equally valid readings to *act on*, and that belongs downstream — here the panel fit supplies azimuth, scale and phase, and the icon supplies the sign of the elevation a candidate is rendered at. Nothing about the fit needed to change; the caller did.

Two habits fall out. Before parameterising a solver to prefer an answer, write the two cases down and check whether they map to the same measurement exactly — if they do, the parameter is inert whatever its tests say, because a unit test built from the same wrong model agrees with it. And an inert option is worse than none, because it reads as a capability: it was removed rather than left as a knob, and the equivalence is now a test so the inert version cannot be added back.

**Anchor:** `PanelFace` and `solveAxonometricFromLattice` in `packages/rendering/src/camera-fit-lattice.ts`; regressions "gives the same lattice for a below-view as for an above-view at negated azimuth" and "fits a below-view panel as an above-view without ever failing" in its test file. Measured 2026-08-06 over the forty panels of `output/camera-fit/score.json`, where a second below-view fit returned null on all forty.
