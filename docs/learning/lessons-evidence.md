...
## A timing ceiling on a worst-case sample measures the machine, not the code

`candidate-lab.spec.ts` sampled a 10ms timer while four candidates generated, took the largest gap between ticks, and required it under 200ms. Twelve runs of the spec alone gave a median of 93ms and a worst of 164ms; inside a full `npm run verify`, where the machine is busier, the same assertion saw 217.6ms and failed the run. This was not a regression — it was checked against a dependency bump landed alongside it, and the built bundles were byte-identical across both lockfiles, so the page ran the same code in every one of those runs. What moved was the machine.

Measuring that directly, over 121 completed runs across load regimes from idle to four times oversubscribed on a 32-core machine: the worst gap ranges 78.8–371.4ms, a factor of 4.71, on one unchanging bundle. A ceiling safe at every load would have to sit at 4.19x what an idle machine does. The 200ms one failed 78 of the 121 runs while failing none of the 12 idle ones, which is the shape of a gate that passes on a quiet machine and is decided by scheduling luck on a busy one.

Both obvious repairs fail on the same data. A quantile is not available: the probe collects 26–32 samples in the ~580ms an idle generation takes, so `ceil(0.99 * n) === n` and p99 is arithmetically the maximum — the same single sample, relabelled. And total blocked time above a 50ms budget is the maximum in disguise, because an idle run puts only about two gaps over 50ms, so the sum tracks `maximum - 50`; under load many gaps cross the budget at once and it ranges 22.97x, the worst behaviour of anything measured.

What survives is a ratio. The share of the sampled window spent over a 16.7ms frame budget ranges 0.190–0.656 across the same 121 runs, needing a ceiling of only 1.53x idle behaviour against the maximum's 4.19x, and it is the only statistic tried whose busiest and quietest runs overlap at all. Load stretches the generation window and the blocked time inside it together, so dividing one by the other cancels most of the machine out. Dropping the budget from 50ms to 16.7ms is what makes the sum a sum: about 15 gaps clear it per idle run instead of two.

The generalisation: when a timing gate must run on a machine you do not control, do not assert on an extreme of the sample, and do not assume a sum is steadier than a maximum — check whether it is dominated by the same one observation. Ask what the number divides by. A statistic whose numerator and denominator both scale with load is the only kind whose threshold can be tight on a quiet machine and safe on a busy one.

**Anchor:** `BLOCKED_FRACTION_CEILING` in `apps/web/e2e/responsiveness.ts`, carrying the calibration table; asserted in `apps/web/e2e/candidate-lab.spec.ts` and unit tested in `apps/web/e2e/responsiveness.test.ts`. Measured 2026-08-07; 14 acceptance runs at the load that produces the highest fractions all pass, where the replaced 200ms ceiling would have failed 10 of the same 14.
