...
- Naming an ambiguity is not resolving it: a check both answers satisfy cannot fail on the swap it exists to catch. ([evidence](lessons-evidence.md#naming-an-ambiguity-is-not-resolving-it))
...
- An open contour has no one-sided pixel test: the stroke straddles the boundary, so rank by the printed line explained. ([evidence](lessons-evidence.md#an-open-contour-has-no-one-sided-pixel-test))
...
- Closed questions are easier to audit: two blind raters agreed on 84/84 pairs; open pick-one-of-six was 39.9% self-consistent. Agreement is not correctness. ([evidence](lessons-evidence.md#the-shape-of-the-question-decides-what-a-vision-call-is-worth))
...
- The structural hash covers the pinned truth, so a catalog bump moves every baseline; restore the old truth and re-hash to see if a placement moved. ([evidence](lessons-evidence.md#the-structural-hash-covers-the-pinned-truth-so-a-catalog-bump-moves-every-baseline))
