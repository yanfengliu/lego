...
- A timing ceiling on a worst-case sample measures the machine, not the code; gate a ratio whose numerator and denominator both scale with load. ([evidence](lessons-evidence.md#a-timing-ceiling-on-a-worst-case-sample-measures-the-machine-not-the-code))
