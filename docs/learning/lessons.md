...
- Absence needs its own outcome: without one a check reports green, and a classifier reports a confident wrong verdict. ([evidence](lessons-evidence.md#a-check-that-has-stopped-checking-still-reports-green))
...
- Local part frames can be right while world placements are mirrored, and a basis is one thing: fixing the position without the rotation scores worse. ([evidence](lessons-evidence.md#local-frames-can-be-right-while-world-placements-are-mirrored))
...
- A score's reachable maximum is a property of the picture; a bar that ignores it measures the panel rather than the placement. ([evidence](lessons-evidence.md#a-scores-reachable-maximum-belongs-to-the-picture-not-to-the-placement))
- A pixel measurement carries the raster it was taken on: convert it through that raster's projection, not through a fit from another one. ([evidence](lessons-evidence.md#a-measurement-converts-through-the-raster-it-was-measured-on))
