...
A bounded camera-hand primitive now represents the as-fitted and x-reflected lattice hands and registers all four quarter turns in each hand by binary silhouette. On the chiral synthetic fixture the unique winner is x-reflected turn 90 at shift `[17,-23]`, IoU 1, while the best proper-hand score is about 0.476; a symmetric 2x4 plate reaches IoU 1 in both hands and returns `camera-handedness-unresolved` while retaining all eight hypotheses. This is a library measurement, not a real-booklet repair: the runner still uses its proper-only camera preparation and does not bind the selected hand into candidate identity, arrow evidence, fixed actions, or retained lineage, while reflection also reverses depth and therefore cannot establish RGB, occlusion, or a physical branch from silhouette alone.
...
| Placement and panel scoring | Experimental, partial | Candidate enumeration, deterministic scoring, and a bounded branch-aware N/N+1/conditional-K path run on the real-booklet driver; one exact attested step-5 shortcut scores its two known origins directly at K, and a tested eight-hypothesis camera-hand registration primitive exists, but the runner still uses the proper-only camera path while the generic farther path carries the intervening step and refuses on shared aggregate budgets. |
...
| Synthetic camera-hand registration | X-reflected turn 90, shift `[17,-23]`, IoU 1; best proper hand about 0.476 | The chiral fixture separates all eight silhouette hypotheses, while a symmetric 2x4 plate ties both hands at IoU 1 and is refused; this primitive is not integrated into the real-booklet runner. |
...
Fixed-ledger omissions and `multi-build-copy` actions still place their raw official transforms because the runner has no resolved search-frame state. Mixing those rows with searched rows under a non-identity proper frame creates a hybrid document that the audit correctly refuses; handedness repair must establish one frame state and map fixed actions through it before full completion.
...
The driver also cannot claim overall completion after placement alone. The deterministic proper-frame audit now names reflection as a target-equivalence failure, and the pure eight-hypothesis primitive can measure a silhouette hand, but neither is integrated as a retained physical-frame decision; the required final Node-side visual audit is also unimplemented.

## Ordered work

1. Integrate the tested eight-hypothesis hand-and-turn registration into `prepareRunStepCamera`, carry hand, determinant, turn, and translation in branch state, candidate IDs, and retained lineage, interpret arrow evidence in that frame, and map fixed-ledger and `multi-build-copy` actions through the same state. Use bound asymmetric booklet evidence, including depth, RGB or occlusion where silhouette is insufficient, rather than official transforms to select a physical branch; retain exact cross-hand ties and never normalize a reflection into canonical output.
