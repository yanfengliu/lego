...
The retained six-part 6651557 pilot is the measurable intermediate for that admission: six exact LDraw closures (70 unique files), checked file-by-file against the complete exact 439-record source audit, cover six designs while independently rehashed Builder native record slices cover five, and its pressure rows show where fractional bounds, non-upright connectivity and oriented collision exceed the current representation. It remains local measurement evidence only; the scored comparison of two distinct source strategies is done and recorded in `part-model.md`, and the fractional-bounds pressure it named is now answered by an opt-in exact bound representation — signed integer units at a fixed 10^-9 scale, bounded on scale and magnitude, absent from every part that does not declare it, so all 77 definitions and the five truth digests are unchanged. Non-upright connectivity and oriented collision still exceed the representation, and no part is admitted yet.
