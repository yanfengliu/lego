...
Nothing consumes it. `deriveTransitionPanelFeatures` detects the icon exactly and records it as evidence, an earlier pass correctly concluded it annotates the viewpoint rather than naming the action, and correctly refused to map it to the `rotation` action class — and then no viewpoint state was ever built. `transition-classifications.json` holds 25 attachments and one final-view, zero rotations, nothing before step 44.
...
The icon count is also an undercount. Attribution requires the icon's centre to fall inside `panel.bounds`, and step 8's sits above its artwork where those bounds do not reach, so page 13 prints two icons and only step 7's is seen. The recorded "39 icons, one per page" is that assumption stated as a measurement.
