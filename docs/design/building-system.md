...
`3020;L` is not. `3020-L-android.bundle` is in the capture. Deriving its frame is the same ordinary work the other fourteen rows went through, and the only reason it has not been done is that `extract-builder-shell.py` admits exactly its fourteen reviewed tuples and `discover-builder-shell.py` is hard-pinned to design 3245 revision M — extending both is a reviewed source change, which `real-build-inputs.spec.ts` deliberately refuses to make for itself.
