...
Still open: nothing yet reads `panelFace` when rendering a candidate, so the run's own comparison is still face-blind. That is the next change, and it is what turns this from a measured fact into a placed brick.
