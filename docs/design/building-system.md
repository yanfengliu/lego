...
**The build places its first two pieces.** Printed step 1 now reports `complete/deferred-lookahead 2/2 placed`, which is the first printed step this repository has ever completed from the booklet's own pixels. The frontier has moved to printed step 2. Everything below that says "zero pieces" describes a twelve-step request, which is still zero for the reasons that section names; the three-step prefix is where the placement now happens.
...
**The refusal is calibrated on wrong answers, not on the right one.** `DEFERRED_STEP_MINIMUM_MARGIN` is **0.0878**, the largest margin any *wrong* pick achieved in that probe: the cumulative panel-2+3 delta score picked branch 1 at 0.0878, the greedy panel-2 delta picked branch 0 at 0.0365, the greedy panel-3 delta picked branch 2 at 0.0212, and `anchorIou` at panel 3 picked branch 0 at 0.0168. The one discriminator that picked branch 3, and picked it at both panels, cleared 0.2085 and 0.0622. The bar is required strictly, so every recorded wrong pick fails it and the recorded right pick clears it by 2.38x. A threshold no wrong answer ever fails is decoration.

**That calibration does not survive review, and the sentence above overstates it.** Three of the four numbers it maximises over are margins of `bestScore` — the next step's single-piece placement score against the next panel's printed highlight, spanning 0.076 to 0.514 on this data — and a fourth is `anchorIou`. The gate they set bounds `registerPrefixAgreement`, a prefix-silhouette IoU against already-built art, which the same probe records as `definedIou` spanning 0.507 to 0.903. **It is a maximum taken over a different quantity than the one it gates**, and over whichever discriminators one probe happened to compute, with no stopping rule: compute a sixth and the bar moves. In the adopted metric the probe recorded *no* wrong-pick margins at all, because that discriminator never picked wrong.
And in its own metric the bar refuses a measured right answer. The two right-pick margins it produced are 0.2085 at panel 2 and **0.0622 at panel 3** — the second is below 0.0878, so a deferral reaching one panel deeper than this one refuses the correct branch. The line above reads as though both cleared; only the first does. False refusal on the only correct margins ever recorded is one in two.
So the two pieces are placed and the branch is corroborated, but the threshold that admitted them is not calibrated — it is a number that happens to sit between this case's right answer and this case's wrong answers at depth one. Re-deriving it in the metric it actually gates, over margins from more than one step, comes before the deferral is trusted at any greater depth.

The three refusals it can produce are named separately: `deferred-panel-unscored` when the panel deferred to has no already-built art or no face-corrected camera — the honest limit of a one-step lookahead, written before it could bite; `ambiguous-deferred-placement` when the margin is not cleared; and `resource-budget-exhausted` when the whole-step product exceeds `deferredCandidateBudget` (512), refused rather than truncated.

`real-build-deferral.test.ts` drives the same decision procedure over real enumerated geometry, through the repository's own camera and point projection rather than a hand-drawn mask: 400 candidates, the drawn assembly first at agreement **1.000** against a runner-up of **0.781**, margin **0.219** — the same order as the 0.2085 the printed panel gave. It also asserts the two refusals from geometry: a panel whose highlight covers exactly where the candidates differ leaves them tied at margin 0 and refuses, and a panel with nothing built drawn on it refuses.

### What step 2 fails on now, which is a different thing

`benchmark-disagreement`, but its content has changed completely. The exhaustive strategy renders 88 candidates and produces a winner; the *pruned* strategy renders **0** and refuses with `incomplete-placement-scoring`, because more than `maxRendersPerPiece` (24) candidates survived the proximity prune and `evaluateSearchBenchmark` refuses a truncated set rather than scoring a prefix of it. So the disagreement is between a strategy that looked and a strategy that declined to look, not between two readings of the panel.

Two things the probe already measured about step 2 are upstream of that and will not be fixed by raising a budget.
Printed step 2 is **also drawn exploded** — the highlighted wedge floats below the assembly with two arrows pointing up into it, and its leading arrow displacement is `[0,-24,0]` at `errorStuds` 0.0428, three plates straight up. So the 527px closed contour outlines a ghost rather than a seat, and no seated candidate matches it however it is placed. The claim below that "an exploded step need not be read at all, because the following step is not exploded" is false on this booklet for step 1 *and* step 2; step 3 is the first seated panel. The signal separating them is already computed and free: arrows kept is 2, 2, 0 on steps 1, 2, 3. `scoreExplodedStep` exists in `apps/web/src/assembly/exploded-score.ts` for exactly this case and nothing in `real-build-run.ts` calls it.
And the drawn step-2 placement is **not in the candidate set at all**: the official canonical transform is `[20,8,-20] upright-yaw-0`, build-plate level, so the wedge slides in *underneath* the step-1 pair. `enumeratePlacements` seeds origins from free studs plus the build plate, and the build plate only when the document is empty; `freeClutches` is computed and consumed only by `discoverConnections`, never to seed an origin. A part that goes under an existing one is therefore never proposed. The editor accepts the placement with three derived connections when handed it directly, so this is an enumerator gap rather than an unbuildable target.

That ordering matters: a deferral, or any ranking, that runs on a set which does not contain the answer cannot be right for any reason, and its margin is noise. Reachability is the check that belongs before ranking, and it is executable wherever the official transform is known.
...
The measure itself is now printed as soon as a result exists rather than with the end-of-run summary, because everything after that point can refuse and throw, and a number that only prints on runs that did not need it is not a measurement. That relocation has not yet been exercised by a probe run.
