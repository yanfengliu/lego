...
The driver also cannot claim overall completion after placement alone. The deterministic proper-frame audit now names reflection as a target-equivalence failure, but it diagnoses rather than corrects the search camera/world hand, and the required final Node-side visual audit is unimplemented.

## Ordered work

1. Resolve the visual/search camera-world handedness that first diverges from the official ledger at step 3, using visual evidence rather than official transforms to select a branch; establish one explicit frame state, map fixed-ledger and `multi-build-copy` actions through it, and never normalize a reflection into canonical output.
