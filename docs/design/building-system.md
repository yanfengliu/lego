...
What this does **not** yet show is a flipped render on a printed panel, and the reason is that step 1 fails `benchmark-disagreement` at score zero, so steps 2 to 4 are `blocked-by-prior-step` and step 4 — the first underside panel — never renders. A four-step run is the probe that would show it, and it reaches the browser with its inputs accepted and no `panel-face-unknown` on any of the four. The face is wired and checked; step 1 is still the thing in front of it.
