...
Two per-design frames are named here before anything derived through them is trusted further. `80015;E` and `3832;G` do not reproduce their pinned `catalogToBuilderLocalTransform` from the export even under the corrected basis, and `80015;E` is both the origin anchor and half of printed step 1. The step-1 relation the corrected pipeline gives, `[40,-8,-40]`, is buildable and interlocked, but it is not the export's `[100,-8,0]`; the difference is the anchor's local origin, not the basis. Settling that frame is the next thing this section owes.
