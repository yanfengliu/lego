...
**Measured 2026-09-24.** `npm run booklet` reads all 359 printed steps of booklet 6651557 (pages 11-219) and aligns them to LEGO's official model of set 21066: 338 by run order, 18 fitted by repair, and 3 (steps 156-158) unmatched because their repair window hit its node budget. The catalog covers 82 of the model's 172 designs, 77 exactly and 5 through an interchangeable mould, and misses 90; the first missing design is needed at step 51 and the first absent colour (LDraw 47) at step 18. Played back from the official poses, the build is valid through step 1 as exported, and through step 31 (184 parts) with two labelled export-frame corrections (the 77844 pin and a reviewed 80015 origin). Of the 359 steps, 31 are valid, 13 invalid, and 315 blocked by the catalog. Both corrections are still needed: they fix the export's LXFML-to-LDraw frames, upstream of catalog truth; without the 80015 origin playback stops after step 1, and without the 77844 pin after step 25. Every LDraw-to-catalog frame playback uses is catalog `/30` truth (201 parts on mesh asset frames, 428 on the parametric frames `scripts/derive-ldraw-catalog-frames.mjs` measures from the official LDraw files), so no ignored run file is read; the retired first-50 frame registry agrees on all 66 of its rows when compared under `LEGO_RUN_EVIDENCE=1`. Step 29, which failed `DISCONNECTED_ASSEMBLY` until `/30`, now passes: the 15573 jumper gained the centre seat that grips a stud half a pitch off its grid. Step 32 then fails `PART_STUD_BODY_COLLISION`. Step 45 is the first the catalog blocks (4519's half-LDU origin). `node scripts/identify-booklet.mjs` identifies callouts by closed-set matching against the inventory, with no model call: 313 distinct drawings, a proven-optimal assignment, and 72 of 74 judged Step 1-50 callouts right, 74 of 74 with the 2 recorded truth errata. Camera fit and placement are not scored yet. Reproduce from the main checkout with `BOOKLET_PDF=recipes/6651557.pdf BOOKLET_LXFML=output/official-model/vx1087034_21066_a.xml npm run booklet` and `LEGO_BOOKLET_PDF=recipes/6651557.pdf node scripts/identify-booklet.mjs`; those paths are also both tools' defaults.
...
- **Catalog `/30` and frame truth** (built): 106 definitions: 61 parametric, 45 mesh-backed; `/30` adds no part. Every parametric part's LDraw-to-catalog frame is catalog truth, measured by `scripts/derive-ldraw-catalog-frames.mjs` from the byte-pinned official LDraw archive. The 15573 jumper gained a centre underside seat. Truth `sha256:cf2d67907369f85551055665b6df0f849dd978d2e63330130ec2d2db8f6ccc0b`, catalog `sha256:4662bd517d807a952bda5fc3964c02e34e0ae502255f747635b6c969fb564ebc`. [`part-model.md`](part-model.md#current-catalog) owns the roster.
- **Migration** (built): Saved documents migrate through explicit versioned steps with a report. `/29` to `/30` carries forward saved `/4`-`/29` edges on a 15573 grid clutch (`undersideClutch:0:0` and `:0:1`) under a proven delta class: 56 carried endpoint deltas across 28 rows. Other removed or changed endpoints still refuse rather than being reinterpreted.
...
- **Alignment to the official model** (built, 3 steps unmatched): 338 by run order, 18 by repair; steps 156-158 are unmatched because their repair window hit its node budget.
...
- **Closed-set identification** (built): `apps/web/src/instructions/identify/` and `node scripts/identify-booklet.mjs`: pdf.js, image-XObject cut-out, matching to 276 inventory thumbnails, min-cost flow plus branch and bound, no model call.
...
- Steps 156-158: not aligned to the official model.
