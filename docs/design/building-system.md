...
**At a three-step prefix the booklet-side failures are gone entirely, and the frontier has moved off the booklet.** Regenerating the inputs at `LEGO_REAL_BUILD_LAST_STEP=3` reports "0 remaining evidence failures through printed step 3", and the run then carries **one** input failure instead of seven or twelve. It is not about the booklet: `input-digest-mismatch`, because the materialized source-mirror renderer could not load `packages/brick-kernel/src/index.ts` over `/@fs/` — `Failed to fetch dynamically imported module`. It reproduces across runs, so it is not a collision with concurrent work.
Nothing had reached that check before; the booklet failures preempted it every previous time, so clearing them exposed the next one rather than causing it. Static inspection rules out the obvious causes: the mirror is byte-complete against the working tree (rendering 16/16, brick-kernel 29/29, catalog 25/25, protocol 3/3 non-test files), the `@lego-studio/*` package aliases are materialized, `@vitejs/plugin-react` is present, and the mirror root carries the `workspaces` field that vite's workspace-root search needs before `server.fs.allow` will cover it. What has not been tried is serving that mirror directly and fetching the module, which is the next diagnostic.
Steps 1 to 3 are what this unblocks, and all three are studs-up panels, so the first placed pieces do not depend on the flip work. Coverage puts four pieces across them: `corner-plate-5x5-quarter-ring` and `corner-plate-4x4-round` at step 1, `wedge-plate-4x4-cut-corner` at step 2, `wedge-plate-6x6-cut-corner` at step 3.
...
**Before any of that, the loop is scoring against the wrong face of the model.**
...
**Two thirds of it is now fixed, with numbers.**
...
The transformation is `(-A, -e)`, not `(A, -e)`, and it is measured rather than derived: because the fit reports `-A` when the true view is below, both signs flip. Rendering a deliberately asymmetric model — a 6x6 plate with a red 2x4 at one corner and a green 1x1 at another — under all four sign combinations shows `(-A, -e)` giving the underside with the two markers correctly swapped relative to the viewer, and `(A, -e)` giving neither.
What remains ambiguous after that is the azimuth branch, because a square lattice is invariant under quarter turns: the fit pins the azimuth only modulo 90 degrees with axis swaps, which is visible in the run itself as steps 1-9 fitting at 55.1 degrees and steps 10-15 at 34.8, where 34.8 is 90 minus 55.2. That is a candidate set of four, not an unknown, and it is resolved the way the rest of this section resolves things — render each and let the panel score them.
...
Still open: nothing yet reads `panelFace` when rendering a candidate in the *printed* run, so `real-build-run.ts` is still face-blind. The synthetic drive above is the proof that wiring it is worth doing and that the rest of the loop needs no other change.
