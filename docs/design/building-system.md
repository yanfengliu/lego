...
**Measured 2026-09-24.** `npm run booklet` reads all 359 printed steps of booklet 6651557 (pages 11-219) and aligns them to LEGO's official model of set 21066: 338 by run order, 18 fitted by repair, and 3 (steps 156-158) unmatched because their repair window hit its node budget. The catalog covers 82 of the model's 172 designs, 77 exactly and 5 through an interchangeable mould, and misses 90; the first missing design is needed at step 51 and the first absent colour (LDraw 47) at step 18. Played back from the official poses, the build is valid through step 1 as exported, and through step 39 (265 parts) with three labelled export-frame corrections (the 77844 pin, a reviewed 80015 origin, and a reviewed 41682 quarter turn). Of the 359 steps, 39 are valid, 5 invalid, and 315 blocked by the catalog. The corrections fix the export's LXFML-to-LDraw frames, upstream of catalog truth; without the 80015 origin playback stops after step 1, and without the 77844 pin after step 25. Every LDraw-to-catalog frame playback uses is catalog `/31` truth (198 parts on mesh asset frames, 428 on the parametric frames `scripts/derive-ldraw-catalog-frames.mjs` measures from the official LDraw files), so no ignored run file is read; the retired first-50 frame registry agrees on all 66 of its rows when compared under `LEGO_RUN_EVIDENCE=1`. Step 29, which failed `DISCONNECTED_ASSEMBLY` until `/30`, now passes: the 15573 jumper gained the centre seat that grips a stud half a pitch off its grid. Steps 32-39, which failed `PART_STUD_BODY_COLLISION`, pass since `/31` gave seven measured parts the `nominal-stud-tube/1` connection profile their studs lacked. Step 40 fails `DISCONNECTED_ASSEMBLY`: with its frame corrected, 41682's centred flange is a ledge whose recessed back face takes a 1 x 2 plate's studs, but the catalog part has no clutch on that face and its collision boxes fill the recess. Steps 41-44 add no issue of their own. Step 45 is the first the catalog blocks (4519's half-LDU origin). `node scripts/identify-booklet.mjs` identifies callouts by closed-set matching against the inventory, with no model call: 313 distinct drawings, a proven-optimal assignment, and 72 of 74 judged Step 1-50 callouts right, 74 of 74 with the 2 recorded truth errata. Camera fit and placement are not scored yet. Reproduce from the main checkout with `BOOKLET_PDF=recipes/6651557.pdf BOOKLET_LXFML=output/official-model/vx1087034_21066_a.xml npm run booklet` and `LEGO_BOOKLET_PDF=recipes/6651557.pdf node scripts/identify-booklet.mjs`; those paths are also both tools' defaults.
