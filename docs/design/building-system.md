...
**Note (2026-09-25, milestone 2b):** the manual editor the capability matrix below calls built was deleted; the build player (`apps/web/src/player/`) and the booklet pipeline are current. This document is not rewritten for that change.

## Executive status

**Measured 2026-09-24 (frontier 3: catalog `/32` and sub-build attach timing, on identity alignment).** `npm run booklet` reads all 359 printed steps of booklet 6651557 (pages 11-219), identifies every callout's part with the closed-set identifier below (no model call, no answer key), and aligns each step to LEGO's official model of set 21066 by those parts. 316 steps match by identity (303 by run order, 13 re-laid by repair; 26 print no callouts), 43 match with their 65 flagged callouts counted instead (count fallback), and none mismatch. Aligned by callout counts alone, as before, 338 match by run order, 18 by repair and 3 (steps 156-158) not, and 22 steps get other parts than identity gives them: step 31 got two dark bluish grey 1x3 bricks that the booklet draws in step 32. Scored against the model, identification agrees with its run order on 313 of the 333 steps that print callouts; the other 20 differ only where the booklet moves a part to another step than the model builds it in (44 pieces), and after alignment all 859 callouts agree. The catalog covers 82 of the model's 172 designs, 77 exactly and 5 through an interchangeable mould, and misses 90; the first missing design is needed at step 51 and the first absent colour (LDraw 47) at step 18. Played back from the official poses, the build is valid through step 1 as exported (2 parts; step 2 fails `PART_BODY_COLLISION`), and through step 44 (280 parts) with three labelled export-frame corrections (the 77844 pin, a reviewed 80015 origin, and a reviewed 41682 quarter turn). Of the 359 steps, 44 are valid, none invalid, and 315 blocked by the catalog. Every LDraw-to-catalog frame playback uses is catalog `/32` truth (198 parts on mesh asset frames, 428 on the parametric frames `scripts/derive-ldraw-catalog-frames.mjs` measures from the official LDraw files), so no ignored run file is read; the retired first-50 frame registry agrees on all 66 of its rows when compared under `LEGO_RUN_EVIDENCE=1`. Step 29, which failed `DISCONNECTED_ASSEMBLY` until `/30`, now passes: the 15573 jumper gained the centre seat that grips a stud half a pitch off its grid. Steps 32-39, which failed `PART_STUD_BODY_COLLISION` under the prior catalog, pass since `/31` gave seven measured parts the `nominal-stud-tube/1` connection profile their studs lacked. Steps 40-44, which failed from step 40 on (`DISCONNECTED_ASSEMBLY` and four body collisions) under `/31`, pass since `/32` gave 41682 the two clutch seats in its wall's back recess, where its ledge takes a 1x2 round plate's studs, and measured its collision as solid intervals, so the recess is empty and no box stands 1 LDU proud of the back face. Step 45 is the first the catalog blocks (4519's half-LDU origin). Playback attaches each sub-build at the printed step the booklet joins it (`tools/booklet/sub-build-attach.ts`), so step 31's sub-build joins in step 31 (page 35), not 32. The app-playable reference build (below) holds all 44 valid steps. It had stopped at 37 for a reason in its own one document, which draws each sub-build where it ends up: the strip the booklet builds over steps 38-43 and joins at 44 touches nothing else until step 40, when the four 41682 brackets, which also sit on the model, take its round plates' studs in their new recess seats. A file ending at the old valid-through 39 was therefore disconnected (257 parts and 8 apart), and the LDraw exporter, which writes one connected document, refused it. The editor shows steps 38-39 as a subassembly and 40-44 as verified; `status.json` records the file's range and any reason it stops early. `node scripts/identify-booklet.mjs` identifies callouts by closed-set matching against the inventory, with no model call: 313 distinct drawings, a proven-optimal assignment, and 72 of 74 judged Step 1-50 callouts right, 74 of 74 with the 2 recorded truth errata. Camera fit and placement are not scored yet. Reproduce from the main checkout with `BOOKLET_PDF=recipes/6651557.pdf BOOKLET_LXFML=output/official-model/vx1087034_21066_a.xml npm run booklet` and `LEGO_BOOKLET_PDF=recipes/6651557.pdf node scripts/identify-booklet.mjs`; those paths are also both tools' defaults.
...
- **Reference build in the app** (built (G3b)): `npm run booklet` writes the valid prefix to ignored `output/booklet/reference-build.mpd`, with one editor step per printed step and labelled as a reference. Use the editor's Import, then Build, to step through printed steps 1-44, every valid step; the file draws a sub-build where it ends up, so the editor calls steps 38-39 a subassembly (the strip is apart until its brackets land at 40). Step 31 holds the booklet's sub-build (a 2x4 plate, two black 1x2x2 bricks and a light grey 1x2 plate) and joins it in that step. `status.json`'s `referenceBuild` records the steps the file holds and any reason it stops early; `apps/web/e2e/reference-build-playback.spec.ts` (opt-in) walks exactly those steps, checks each step's counts against the printed callouts, and fails on a file cut short with no recorded reason. Looked at 2026-09-24 in the editor served from the change's own worktree, through Import, Build and Next step, from the default angle and from behind: steps 31-44 match booklet pages 35-45 part for part, unmirrored; the strip of steps 38-44 stands on its side on the upper deck, axle holes up and white slopes forward, as page 45 draws it; trans-clear appears as an opaque stand-in.
...
- Step 45: the first step the catalog blocks, on 4519's half-LDU origin; steps 1-44 are valid and the app-playable reference build holds all of them.
- The reference build is one LDraw document, which the exporter writes only when its last state is connected, so a valid prefix that ends while a sub-build is still apart in it is cut back, with the reason recorded. Sub-builds as submodels would lift that limit.
...
1. Step 45: 4519's half-LDU origin (below, G3a-3), now the first step playback cannot place. Steps 40-44 are done: catalog `/32` seated 41682's recess and emptied its collision there.
...
5. Catalog `/31` and `/32`: done. Seven measured parts gained the `nominal-stud-tube/1` connection profile and 41682's export frame was turned to the ledge the booklet draws (`/31`, valid through 39); 41682 gained its two recess seats and solid-interval collision (`/32`), and sub-builds attach at the printed step the booklet joins them: valid through 44, and the reference build holds all 44.
