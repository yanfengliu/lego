...
The following twelve definitions still fail because `semantic-tube-seat-grid` declares clutch seats but draws no underside shell:
...
The complete matched-view rule was established during `/12` review. `80015` has a full named directional packet; `30503`, `6106`, and `30565` have top, underside, isometric, and shared printed-step evidence but not complete front, back, left, right, and underside-oblique packets. Those missing views are explicit review debt, not evidence supplied by omission.

Before any catalog-truth version after `/12` may enter, every new or modified part must satisfy all of the following. The twelve current `underside-is-drawn` failures predate the complete standard and remain grandfathered geometry debt; they must be repaired before `parts:check` joins `verify`, and the three incomplete `/12` view packets must also be closed.
