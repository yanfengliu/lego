...
The same derivation, run over the fifteen designs the real build's opening printed steps place, made the symmetry problem a protocol rather than a case-by-case judgement, because most of those parts are plain plates and a plain plate is symmetric.
A 4x6 plate's stud set is unchanged by a half turn and an 8x8 plate's by every quarter turn, so "exactly one exact frame, or refuse" could never calibrate a plate at all; the rule is now that the exact candidates are quotiented by the catalog part's own *measured* self-symmetry — the residual has to map every connector, every collision primitive, every clutch allowance, both bounds and the footprint dimensions onto themselves — and anything symmetry cannot explain has to beat its runner-up by at least a factor of four against the independent LDraw surface.
That is a tightening, not a loosening: what used to be accepted was a bare count of one, and what is accepted now is a proof that the residual is unobservable or a measured margin that says which frame the geometry agrees with.
Nine of the fifteen are settled by symmetry with nothing to choose between the candidates, five are unique on the studs alone, and 51739 is the one that needs the witness: its four studs are a square and it is a wing, so all four quarter turns fit and only the surface separates them, by 27.4x.
A part the proof cannot describe — a bundled source mesh, a collision body it cannot rotate — is treated as *not proven* rather than as symmetric, so it falls through to the witness instead of being quotiented away.

Rotated fields settled a question the pilot recorded as open.
`Custom2DField` stores its rotation as nine numbers whose row-major and column-major readings differ for anything but a symmetric matrix, and every pilot field carried the identity, so nothing there could say which order Builder writes.
Two of the fifteen — 30503 and 6106 — carry a quarter turn, and both readings put the studs on an exact lattice, so exactness alone still cannot choose; the Builder Shell can, and it does by 87x and 123x in favour of **column-major**.
The XML's own redundant `angle` and `ax,ay,az` attributes agree independently: 2310's `angle="120" ax=ay=az="-0.5773"` is the -120 degree rotation about (1,1,1), which is the transpose of what its nine numbers read row-major.

41769;G is the one design in those printed steps with no frame, and the reason is a source, not a derivation.
Its served bundle hashes md5 `fb1e8bb3edf0174350cf84b75a378b6a` against the `cab7c4020d384b66e079c5c86bb40f03` the Builder manifest itself declares, so it sits in the quarantine the coverage ledger already records and has never been decoded.
Its catalog part `builtin:wedge-plate-2x4-right` exists and its type-23 field is readable from the asset audit; what is missing is the Shell mesh the surface corroboration needs, and a frame with no independent witness is exactly what this protocol refuses.
`3020;L` used to be the other design with no frame and no longer is, and the contrast is the whole reason 41769;G stays refused: 3020's bundle *is* in the capture and *does* reproduce the md5 its own manifest row declares, so the ordinary derivation applies to it, and it is pinned at `[30,-4,10] upright-yaw-90` by the plate's own symmetry at 1.305568 LDU against the LDraw surface.
