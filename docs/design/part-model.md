...
The builtin catalog is `builtin.basic-parts/19` with 91 definitions. Sixty-one render from repository-authored parametric recipes and 30 render bundled, content-hashed LDraw surfaces with source-faithful normals. Sixteen of the mesh-backed definitions are render-only promotions of earlier parametric parts; the other fourteen are fully measured definitions.
...
The tracked booklet has 121 distinct required leaf design identities. That is not a 91-of-121 coverage fraction: catalog definitions include parts outside the set, aliases and design identities do not share one denominator, and some booklet callouts remain unidentified. A deterministic current-catalog intersection now finds 13 of those 121 identities, digest `sha256:e5f3931d7bc1908df68dcae51df9c044bef6e8a72a216abbe3df6aa087706452`; `/19` adds exactly `41682`, whose first printed use is step 40. Full-booklet manifest v6 corrects printed step 18's quantity-two callout from the adjacent `3069` crop to element `6514469`, design `25269`; exact source-component replay independently reproduces that corrected quantity-two evidence, and `/14` carries the measured quarter tile. Fresh trusted crop-local model adjudication is still absent, however, so there is no current v6 score or coverage prefix: 73 current-input replies captured before the local-write transport defect was found remain quarantined under legacy answer schema `/4`, and strict `/5` refuses them. `/15` admits distinct design `28802` from its official LDraw and LDCad evidence, but the printed-step-26 Builder source still contradicts that identity by naming `10201`; the additive `/16` `35787`, `/17` `11253`, `/18` `15254`, and `/19` `41682` rows likewise do not resolve step 26 or advance the retained action ledger, whose source-boundary check reaches step 25 only by retaining omissions and refusals rather than claiming every intervening piece is placeable.
...
`npm run parts:check` covers all 91 definitions and is part of `npm run verify`. It is a declaration-consistency gate, not a substitute for source review, matched visual inspection, connector evidence, collision review, or browser verification.
...
The current `/19` admission tranche closes the new `41682` definition against its exact 14-file official LDraw closure. Packet `sha256:6265de5e960fdb4d9b839ebe1f36a046f6f8b95626965ffbda1a7dc50fcfb7ec`, capture batch `sha256:d7005fc30dd7fa84acadb64012efaa1c8023f6af8720f8bdf0f5e746dd680d18`, native-pair manifest `sha256:b382da75b01ea732d79a303c706dbba30d29fdabc5d9f46906a9bca322871320`, review `sha256:9b5e29a382f5fa408bd94f1e40e9497358c3ec465d4c8e4d6fad6e7802a0373e`, and review batch `sha256:6860016197eb346118fd039df36ec4086c58d9f46cbe36510535c2dc2e4df922` bind its eight original 640 × 640 source/catalog pairs. Every pair is RGBA-exact and was inspected at original size; the strict front and bottom cameras expose only matching occluding silhouettes, while the side, isometric, and underside-oblique views expose the T-profile, handedness, side-stud occlusion, central tube, wall opening, and hard-versus-smooth shading. The review records all eight as `same`. The other 29 mesh packets remain historical `/18` evidence until recaptured under `/19`; neither tranche certifies hidden interiors, exact physical collision, clutch or minifig fit, sideways attachment, or stability. The matched renderer applies the declared recipe frame to both source and candidate, so agreement does not independently prove the frame; exact source triangles, frame, and connector-route regressions carry that claim.
...
Parametric features derive collision where that derivation is justified. Fourteen fully measured mesh definitions use conservative column height fields; sixteen render-only promotions preserve their earlier conservative recipes. Neither mode proves that every visible cavity is physically usable.
