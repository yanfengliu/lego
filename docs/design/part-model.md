...
The same derivation, run over the fourteen designs the real build's opening printed steps place, made the symmetry problem a protocol rather than a case-by-case judgement, because most of those parts are plain plates and a plain plate is symmetric.
...
Eight of the fourteen are settled by symmetry with nothing to choose between the candidates, five are unique on the studs alone, and 51739 is the one that needs the witness: its four studs are a square and it is a wing, so all four quarter turns fit and only the surface separates them, by 27.4x.
...
Two of the fourteen — 30503 and 6106 — carry a quarter turn, and both readings put the studs on an exact lattice, so exactness alone still cannot choose; the Builder Shell can, and it does by 87x and 123x in favour of **column-major**.
