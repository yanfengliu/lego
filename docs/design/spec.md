...
Builtin catalog `/32` has 106 definitions: 61 parametric and 45 mesh-backed, the latter rendering bundled LDraw surfaces with source-faithful normals. `/30` to `/32` add no part. `/30` makes every parametric part's LDraw-to-catalog frame catalog truth, measured by `scripts/derive-ldraw-catalog-frames.mjs` from the byte-pinned official LDraw archive, and gives `15573` a centre underside seat; `/31` gives seven measured parts' studs the `nominal-stud-tube/1` connection profile; `/32` gives `41682` two clutch seats facing +Z in its wall's back recess and per-column solid-interval collision. Current overall truth is `sha256:dbb4c8147a20b8ed0598ebf617583993fa7dd89788cdc491056d1579ee7b19d6`, with catalog `sha256:2d95bc0b371954519d2f04bc56a950036c63de162469688d62cba9bee4fe2832`. Historical truths stay pinned separately and are never relabeled as current evidence.

The fail-closed migration-history gate (`npm run migration-history:check`) reconstructs 33 reviewed source truths and verifies 2,342 current endpoints, 970 reviewed endpoint deltas, and zero reachable-pair deltas. The `/31`→`/32` report names `41682`, whose two recess seats are additions: a saved edge on its existing ports carries forward unchanged, and one naming a seat its source truth never had refuses. The `/30`→`/31` report names the seven parts whose studs gained the `nominal-stud-tube/1` profile, and carries a saved edge on one of their 26 studs under a second reviewed class the gate proves (the profile is the stud's only change and never exceeds its measured radius); with the `15573` class, the gate proves 476 carried endpoint deltas across 29 rows. The `/29`→`/30` report names the 61 parametric definitions whose LDraw interchange frame it measured, and separately `15573`'s connector and connection-conditioned collision semantics. A saved `/4`–`/29` edge on a `15573` grid clutch (`undersideClutch:0:0` or `:0:1`) carries forward under one reviewed delta class that the gate proves: the connector only gains shared-capacity cells whose other members the source truth did not have. The report lists those carried endpoints; the gate proves 56 carried endpoint deltas across 28 rows. Any other source port that disappeared, moved or changed still refuses.
