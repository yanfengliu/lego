...
Revised 2026-08-23: current implementation includes catalog `/18` with one consolidated current-truth exterior review for all 29 mesh-backed definitions, bounded branch-aware farther-panel carry, one exact source- and data-attested step-5 origin shortcut, a deterministic post-search target-equivalence audit, and the live generation-3 browser boundary that retains the exact eight-way empty-root camera frontier and refuses before the first placement. The production farther-step attempt now runs narrowing through a fixed-8,192 refundable subject-raster ledger and a semantics-preserving 24-bit depth route with dense prefixes, parent-local sparse probe reuse, strict-nearer composition, exact whole-scene fallback on equal-depth ties, and conservative whole-scene batches when an existing magenta part collides with the probe key; the retained historical `/15`-denominator depth control reproduced all 14,172 baseline rows and 17 leaves with 6,559 subject rasters. Standalone authority-absent prerequisites now include parent-bound exact lineages, multi-root atomic compilation, compiled-observation replay, dense branch-role transport, streamed 359-panel source evidence, D4 camera evidence, zero-piece transitions, the fixed-five source-parity request seam, and a bounded canonical Ed25519 broker-receipt verifier whose output explicitly has authority absent. A detached browser-output `/4` envelope and reader cross-bind synthetic roles into evidence-derived reports, failures, identities, and terminal bytes, with source records/arrays non-Proxy, raster primordials checked before derivation, replay timings fixed to zero, and every reported prepared panel required to have no coverage failure, unresolved callout, or missing design. The real 359-step source-parity diagnostic was retained on 2026-08-22 under measurement digest `sha256:39a810d8a30f28fb93bb619f2ce522ce01e95146e8fe14666d6606270c0cb615`, but remains calibration-only with opaque browser commitments and authority absent. A fresh required 1..12 production attempt stopped at input preparation with zero completed steps and zero pieces because no authentic manifest-v6 `/5` identification answers exist, so current coverage and action-ledger evidence cannot bind; it never exercised the browser depth path and grants no step advancement. The detached reader does not enter `runRealBuild`, authenticate source execution or provisional identity, establish a physical frame, accept a user action, mutate a document, or grant placement/completion authority; there is no live `/4` producer, and public browser-output `/3` remains the exact eight-root step-1 refusal when current input closure reaches it. Superseded implementation history lives in the devlog.
...
The 14,172-row reproduction remains the retained historical `/15` long-browser denominator; the `/18` catalog repins its production compatibility boundary but no `/16`, `/17`, or `/18` long rerun is claimed.
...
Part admission uses a dedicated browser harness rather than treating the editor's seven-view presentation hook as review evidence. It materializes a checksum-pinned LDraw closure, renders it through an independent `LDrawLoader`, renders the production catalog definition in a clean detached scene, and captures matched top, bottom, front, back, left, right, isometric, and underside-oblique pairs through one union-fitted camera packet. Generated packets bind sources, frames, renderer policy, cameras, PNG and decoded-RGBA hashes, and diagnostics, but remain `pending` until a separate immutable explicit review records `same`, `different`, or `not-observable` for every view. The historical `/13` through consolidated `/17` tranches remain evidence under their original truth; one consolidated current `/18` batch separately closes all 232 visible pairs for the 29 mesh-backed definitions under collision and validator `/3`. Twenty-eight outcomes moved into new sidecars only after exact old/current image and render-input identity, while all eight `15254` pairs were freshly inspected and the predecessor isometric and underside-oblique pairs received a fresh original-size sweep; exterior agreement still cannot certify a hidden cavity, collision volume, clutch or minifig fit, sideways attachment, or stability.
