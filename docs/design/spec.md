...
Revised 2026-08-12: current implementation includes bounded branch-aware farther-panel carry, one exact source- and data-attested step-5 origin shortcut, and a diagnostic-prefix boundary that withholds canonical completion when target-frame equivalence is unreconciled; superseded implementation history lives in the devlog.
...
A structurally valid searched document is not automatically the official target. Node finalization binds the retained document, reports, identity metadata, searched transforms, build sequence, and structural validation, then withholds canonical output when searched transforms differ from the raw official ledger and no independent proper world-frame mapping has been proved. It may retain the exact candidate bytes only as a `diagnosticPrefix` with `targetEquivalence: "unreconciled"`; in that case canonical `documentJson`, `structuralHash`, and `finalParts` remain null, null, and zero. Reflection is never accepted as a free world-frame change, official transforms are a post-search evaluator rather than an enumerator or scorer input, and a reusable proper-frame equivalence audit remains unbuilt.
...
`building-system.md` owns the measured position of this loop and the ordered plan for what is missing. Outside the exact attested step-5 shortcut, the current runner does not yet produce the complete arbitrary-depth N/N+1/farther observation packet, does not use raster evidence in transition classification, has no integrated vision checker, and does not backtrack. Its target guard refuses an unreconciled raw-frame mismatch, while a separate semantic audit found the current reflection; a reusable proper-frame audit and the search camera/world handedness repair remain open.
