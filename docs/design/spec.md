...
The current repository has one opt-in legacy model-assisted derivation producer that describes placement relations from one printed panel. Part-identification first-pass and re-ask provider execution are disabled: strict `/5` code can construct and replay synthetic local proofs, but cannot launch a real child until the missing authorization and launch-settlement contracts are added; the retired `/4` answers are explicitly non-resumable. A third quarantined local foundation defines source-bound multi-panel checker requests, attempts, results, refusal consumption, and a one-tool subscription-CLI adapter, but it has no real-PDF/candidate-render producer, driver consumer, consent preflight, or successful live verdict. All write or are designed to write ignored development evidence, are not broker-backed or exposed in the web product, are not wired into the real-booklet driver, and have no path to mutate a user document.
...
Consent is scoped and specific under both the current local tooling and any target broker path. Cropped regions of the user's own instruction booklets — callout thumbnails and step panels — may be sent to a model for part identification or printed-step placement description only after explicit authorization. Crops only: never a whole booklet, other repository content, credentials, or another user reference, design, or artifact; retained responses stay under ignored local output roots. The strict part-identification transport therefore remains hard-disabled: no tracked or ignored record yet binds purpose, the exact ordered card/request digests, provider and model, privacy/provider-policy evidence, consent, one-launch scope, and budgets through request, launch ticket, proof and checkpoint. Its sanitized proof intentionally discards raw stream/session data and is `local-diagnostic`, `authenticated: false`, and non-executable; it cannot establish that Anthropic ran, which remote weights or policy applied, or that no process wrote outside the final audited task root. The current panel-placement script intends the crop boundary but accepts any resolved local path without source or image validation, launches the CLI from the repository with repository-wide `Read`, and retains no tool trace, so that boundary is policy rather than an enforced or auditable data limit. The quarantined checker constrains which already-bound bytes its adapter can expose and minimizes model-visible metadata, but a digest plus claimed PDF/page/bounds is not proof of crop origin or consent, and candidate-render transmission also requires explicit scoped authorization. A replayable successor must enforce the exact authorized snapshot rather than merely state the restriction in its prompt.
