...
Revised 2026-08-12: current implementation includes bounded branch-aware farther-panel carry; superseded implementation history lives in the devlog.
...
The deterministic real-booklet driver now integrates a bounded branch-aware farther-panel path for an ambiguous step N: it retains every origin with its cached exact N+1 score render, carries one intervening step under shared candidate and narrowing ledgers, and scores conditional K only after the carry completes atomically. Hash, piece-witness, lineage, report, parser and artifact bindings prevent one branch or image from being relabeled as another. A family decision may reject an origin family but cannot falsely settle an ambiguous descendant, and a typed budget or observability refusal leaves all completed alternatives immutable without changing the document. This is not generic deep backtracking and does not integrate the model checker.
...
Status: active. The internal real-booklet runner can settle and retain a contiguous prefix and can carry a bounded ambiguous origin through one intervening printed step with immutable lineage, aggregate render reservations before work, aggregate child reservations before retention, exact N+1 captures, conditional K, and named numerical refusals. Generic deep backtracking, farther reach beyond that measured path, and one-frame reconciliation with the official ledger remain open. The exact current numbers belong only in `building-system.md` and retained run scoreboards.
