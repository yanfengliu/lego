...
Consent is scoped and specific under both the current opt-in CLI and any target broker path. Cropped regions of the user's own instruction booklets — callout thumbnails and step panels — may be sent to a model for part identification or printed-step placement description only after explicit authorization. Crops only: never a whole booklet, other repository content, credentials, or another user reference, design, or artifact; retained responses stay under ignored local output roots. The current panel-placement script intends this boundary but accepts any resolved local path without source or image validation, launches the CLI from the repository with repository-wide `Read`, and retains no tool trace, so the boundary is policy rather than an enforced or auditable data limit. The quarantined checker constrains which already-bound bytes its adapter can expose and minimizes model-visible metadata, but a digest plus claimed PDF/page/bounds is not proof of crop origin or consent, and candidate-render transmission also requires explicit scoped authorization. A replayable successor must enforce the exact authorized snapshot rather than merely state the restriction in its prompt.
