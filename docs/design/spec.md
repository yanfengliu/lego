...
`building-system.md` owns the measured position of this loop and the ordered plan for what is missing. Outside the exact attested step-5 shortcut, the current runner does not yet produce the complete arbitrary-depth N/N+1/farther observation packet, does not use raster evidence in transition classification, has no integrated vision checker, and does not backtrack. Its deterministic post-search audit now refuses the measured reflection after complete catalog-realization comparison; the search camera/world handedness repair remains open.
