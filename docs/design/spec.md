...
Revised 2026-08-22: current implementation includes catalog `/15` with the separately reviewed `25269` and `28802` exterior packets, bounded branch-aware farther-panel carry, one exact source- and data-attested step-5 origin shortcut, a deterministic post-search target-equivalence audit that withholds canonical completion when no proper upright world frame realizes the official target, and a generation-3 browser boundary that retains the exact eight-way empty-root camera frontier, routes fixed actions through a fail-closed physical-authority guard, and intentionally refuses before the first placement. Stable document-candidate and parent-bound lineage identities, bounded inspection-only lineage evidence, live panel-camera adapters, canonical candidate-document snapshots, a normalized farther DAG, prepared-search inspection and an automatic BuildProgram compiler now exist as prerequisites. A standalone inspection-only atomic compiled-branch batch snapshots detached enumerator offers, retains the exact ordered request and reservation, deduplicates convergent physical work, independently recompiles every unique transition in Node, and emits exact child snapshots plus parent-specific lineage edges. A separate compiled-observation-closure `/1` sidecar replays exact MSB-packed silhouette registration and convergent selection under a prepared policy, while a browser branch-evidence `/1` index binds dense external compiled-evidence and observation roles without base64 expansion. A refusal-only local consumer now rejects orphan closure tables and charges measured cumulative lineage rows, typed observations, source/camera rows, selection/accepted-transition references, exact root/child canonical-document bytes, proposals and raster work before bounded root reconstruction, then derives and charges conservative compiler graph/byte work-policy units including step preparation before semantic replay; typed observation reuse requires the exact replay-admitted work inspection. It exact-binds prepared step/compiler/proposal facts, requires legacy observation-generation, selection, and acceptance fields to remain empty, separately verifies each typed closure and any nonempty committed raw role, and returns an authority-absent summary of local lineage and observation calculations. An authority-free source-parity `/4` diagnostic now retains exact production, candidate, and XOR evidence for five mask classes across a dense 359-step opt-in browser path, with source/runtime, exact PDF, prepared-panel, served-response, browser-result, and publication bindings; its browser work-RGBA, policy, and derivation commitments remain explicitly opaque and its source-RGBA display column is visualization-only. The fixed five-panel path now adds a separate source-bound publication wrapper that reproduces the exact browser capture, full 359-row manifest, source snapshot, provenance, served source, checkout, and runtime closure before deriving an audit-only execution identity and retaining five roles plus ten PNGs under a content-addressed run. It remains pending and authority-free: the review foundation still returns `needs-adjudication`, and no human-owned issuer or admission capability exists. None of these seams forms browser-output `/4`, enters `runRealBuild`, mutates a document, or grants completion; browser-output `/3` therefore remains the exact eight-root step-1 refusal. Superseded implementation history lives in the devlog.
