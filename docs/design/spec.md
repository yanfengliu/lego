...
The exact-five calibration path binds only steps/pages 90/79, 101/87, 346/213, 358/218, and 359/219 to a contract derived from all 359 prepared panels. Its browser-only constructor retains exact high/work RGBA, H/P/D stage bytes, W, pairwise P/D/W facts, and lossless PNGs in five bounded external roles; the Node parser reconstructs the transient wire, decodes every PNG to the retained RGBA, replays H/P/D, and independently re-derives W from the work raster and prepared crop. This capture artifact intentionally carries no execution identity or publication/source-closure binding, grants no authority, and has not been run against the real booklet. The separate review parser still labels human review as an unverified claim, and adjudication remains `needs-adjudication` for missing truth, drift, W mismatch, and exact W equality alike because no human-owned review issuer or execution-to-W provenance boundary exists. It cannot admit or launch the full source-parity probe.
