...
Revised 2026-08-12: current implementation includes bounded branch-aware farther-panel carry, one exact source- and data-attested step-5 origin shortcut, a deterministic post-search target-equivalence audit that withholds canonical completion when no proper upright world frame realizes the official target, and pure panel-camera observation, eight-branch resolution, and fail-closed fixed-action prerequisites that are not yet integrated into the runner; superseded implementation history lives in the devlog.
...
`building-system.md` owns the measured position of this loop and the ordered plan for what is missing. Outside the exact attested step-5 shortcut, the current runner does not yet produce the complete arbitrary-depth N/N+1/farther observation packet, does not use raster evidence in transition classification, has no integrated vision checker, and does not backtrack. Its deterministic post-search audit now refuses the measured reflection after complete catalog-realization comparison. A pure registration primitive scores all four proper quarter turns and all four x-reflected counterparts with a separate translation for each and refuses an exact cross-hand silhouette tie. `RealBuildPanelCameraRegistration` validates, copies, and deeply freezes `{latticeHand,latticeDeterminant,registrationPanelStepNumber,turnDegrees,shiftPx}` as panel-local raster evidence, never as authority for a physical transform. The pure resolver snapshots the canonical document, masks, and shared ledger state around hashing and rendering, verifies the supplied document digest before work, requires every registration panel to be strictly later than the prefix, and atomically reserves all eight angular branches before producing any seed or observation. An empty step-0 root yields eight explicitly unregistered seeds with null shift and observation identity rather than fabricated pixel evidence; a nonempty prefix retains all attempts and successful observations under one hash-bound `candidateId`, unique `lineageId` values, and the supplied `parentLineageId`, while an incomplete render batch selects nothing. Arrow evidence binds the fit and raw measurement to the same panel, derives the q0 family exactly once, and applies the integer D4 map without changing row order or the measured travel and off-line tolerances. Every resolver result leaves physical-frame authority unresolved. A separate fixed-action guard returns typed `fixed-ledger-frame-unresolved` for every current panel-camera, reflected, unresolved, mismatched, or caller-self-certified proper claim and does not even read the guarded executor; no trusted authority producer exists. The real-booklet runner, scorer, reports, farther lineage, and fixed-action call sites do not yet consume these contracts, so the search camera/world handedness repair remains open.
...
Status: active. The internal real-booklet runner can settle and retain a contiguous diagnostic prefix, carry a bounded ambiguous origin through one intervening printed step with immutable lineage, aggregate render reservations before work, aggregate child reservations before retention, exact N+1 captures, conditional K, and named numerical refusals. One source- and data-attested calibration may score the exact two measured step-5 origins directly at panel 7, while the generic path remains conditional on completing its intervening carry. Node finalization now applies the bounded proper-upright target-equivalence audit after exact identity binding and separates structurally valid diagnostic bytes from canonical target completion. The pure `LatticeHand` view transform and eight-hypothesis silhouette registration are implemented with exact cross-hand tie refusal. `RealBuildPanelCameraRegistration` retains hand, determinant, registering panel, turn, and translation as panel-local observation state; its companion resolver freezes and hash-checks the candidate and raster inputs, requires a strictly later panel, reserves eight branches atomically, represents the empty root as eight unregistered angular seeds, and retains observed attempts under a stable hash-bound candidate plus explicit child and parent lineage. Every result remains physically unresolved. The separate fixed-action guard emits `fixed-ledger-frame-unresolved` without reading its executor because caller-supplied data cannot mint the absent opaque physical-frame authority. `prepareRunStepCamera`, scoring, reports, farther lineage, and fixed-action call sites do not use these prerequisites. Corrected search camera/world handedness, a trusted physical-frame authority producer, generic deep backtracking, farther reach beyond the measured paths, and one-frame reconciliation with the official ledger therefore remain open. The exact current numbers belong only in `building-system.md` and retained run scoreboards.
