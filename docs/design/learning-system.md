...
| Candidate lineage, farther-frontier, and backtracking primitives | Frozen candidate-only farther carry implemented; current lineage cutover incomplete | The legacy path retains immutable N parents, exact atomic piece witnesses and N+1 captures, carries one intervening step under shared ledgers, and conditionally scores K only after an atomic carry; current `/3` execution refuses before entering it, and generic deep reversal is not integrated. |
| Camera horizontal-hand registration and observation lineage | Empty-root and fixed-action refusal boundaries integrated; scored lineage carry unbuilt | Browser-output `/3` atomically retains the eight canonical empty-root seeds and then refuses step 1 before placement because scoring is not carried through their lineage IDs; fixed-action call sites enter the opaque guard and refuse before their executors, while deferred/farther candidate lineage and recomputable positive-placement evidence remain unintegrated. |
...
A booklet run reads prepared printed steps, compiles an ordinary step against one settled `BrickDocument` prefix, enumerates candidate placements, and compares a render with a printed panel at that panel's fitted camera and face. The registration primitive can measure both determinant cosets, and `RealBuildPanelCameraRegistration` retains hand, determinant, registering panel, turn, and pixel shift as panel-local observation state without claiming a physical transform. The resolver atomically reserves complete eight-way angular families and keeps a document's hash-bound `candidateId` stable while assigning each seed or observation a unique `lineageId` with an explicit `parentLineageId`. The live runner now creates the exact canonical empty document, atomically reserves its eight unregistered roots, records them in the first browser-output `/3` row, and deliberately refuses step 1 before placement because its candidate scorer and deferred/farther state do not yet preserve those lineage IDs. Later requested rows remain represented as causally blocked, and no branch becomes document truth.
