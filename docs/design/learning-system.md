...
| Candidate lineage, farther-frontier, and backtracking primitives | Current identity and bounded inspection prerequisites implemented; executable carry unavailable | Stable `document:sha256:*` candidates, parent- and origin-bound `lineage:sha256:*` identities, exact canonical-document snapshots and a normalized convergent farther DAG are implemented. Prepared-step/search bytes and lineage evidence produce inspection only; every origin, carry, panel and transition authority issuer remains unavailable until trusted preparation, enumeration, compilation and measurement are composed. The frozen candidate-only legacy path remains separate. |
...
Retained run `2026-08-12T10-28-52-560Z-34694c87c62e-26128a38-4d9e-4294-9091-6aae9b3ca367` is immutable legacy inspection-only evidence under artifact-manifest `/3`, run-contract `/2`, browser-output `/2`, and result/score `/4`. Its exact inspector does not call current browser or finalizer predicates: frozen generation-2 browser, farther, capture, atomic report, completion-failure and identity predicates bind the downstream closure, while pinned semantic and per-step prefix validation reproduces the five-step, eight-part, 30-connection diagnostic document with hash `sha256:47ae3d353885f5de11b685a4bec4ca1132554a19e1f1e30454281252f7d64c93`. That proves which historical diagnostic bytes survived, not that they are a current-generation publication or the official model.
