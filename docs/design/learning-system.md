...
| Camera horizontal-hand registration and observation lineage | Pure panel-camera registration and branch-admission prerequisites implemented | The registration ranks all eight hand-and-quarter-turn silhouette hypotheses and refuses exact cross-hand ties; panel-local registrations, ordered D4 arrow families, separate stable candidate and panel-observation IDs, and atomically pair-budgeted branches exist, but the real-booklet driver, scoring, reports, farther lineage, and fixed-ledger actions do not consume them. |
...
Current gaps are explicit. The run contract does not separately bind a catalog snapshot or complete truth bundle; a step row does not record document revisions, a before-and-after structural-hash pair, inline per-image digests, the horizontal-hand hypothesis set or selected determinant, or a complete arbitrary-depth N/N+1/first-revealing-farther observation packet; and `score.json` does not contain reversal depth, retained byte count, replay level, or the structured target-equivalence audit. The pure prerequisite now separates stable document-node `candidateId` from panel-camera `observationId`, but existing runner IDs, scoring rows, reports, and farther lineages do not call it and remain panel-camera-blind. Replay closure and artifact-size facts live in separate manifest files, the bounded one-intervening-step driver has no generic reversal depth to report, fixed actions remain outside the registration contract, and the implemented post-search audit diagnoses but does not repair the search camera/world hand. Panel-local registration is never physical-transform authority; fixed actions need a separately established physical frame.
