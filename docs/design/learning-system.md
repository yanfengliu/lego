...
| Camera horizontal-hand registration and observation lineage | Pure panel-camera resolver and fixed-action refusal seam implemented | Registration ranks all eight hand-and-quarter-turn silhouette hypotheses and refuses exact cross-hand ties; the resolver atomically retains eight root seeds or observed branches under stable candidate and explicit lineage identities, and the fixed-action seam refuses without separate physical authority, but the real-booklet driver, scoring, reports, farther lineage, and fixed-action call sites do not consume them. |
...
A booklet run reads prepared printed steps, compiles an ordinary step against one settled `BrickDocument` prefix, enumerates candidate placements, and compares a render with a printed panel at that panel's fitted camera and face. The current runner still takes only its as-fitted horizontal hand. The separate pure registration primitive can measure both determinant cosets, and `RealBuildPanelCameraRegistration` can retain the hand, determinant, registering panel, turn, and pixel shift as panel-local observation state without claiming a physical transform. The resolver requires a panel later than the prefix, atomically reserves all eight angular hypotheses, and keeps the document's hash-bound `candidateId` stable while assigning each seed or observation a unique `lineageId` with an explicit `parentLineageId`; N+1 or K can therefore append evidence without rekeying that candidate. The empty step-0 root is represented by eight unregistered seeds with no observation ID or shift. None of these contracts is in the run path. A deferred origin may now branch temporarily, but no branch becomes document truth until the bounded family decision is complete.
...
Current gaps are explicit. The run contract does not separately bind a catalog snapshot or complete truth bundle; a step row does not record document revisions, a before-and-after structural-hash pair, inline per-image digests, the horizontal-hand hypothesis set or selected determinant, or a complete arbitrary-depth N/N+1/first-revealing-farther observation packet; and `score.json` does not contain reversal depth, retained byte count, replay level, or the structured target-equivalence audit. The pure resolver now separates a stable hash-bound document `candidateId` from seed and observation lineage, but existing runner IDs, scoring rows, reports, and farther lineages do not call it and remain panel-camera-blind. Replay closure and artifact-size facts live in separate manifest files, the bounded one-intervening-step driver has no generic reversal depth to report, and the implemented post-search audit diagnoses but does not repair the search camera/world hand. Panel-local registration is never physical-transform authority. The fixed-action guard makes that refusal executable with `fixed-ledger-frame-unresolved`, but no current fixed-action call site uses it and no trusted authority producer exists.
...
Those rules are implemented in the generic build-tree and backtracking libraries. The real-booklet driver now consumes the bounded farther-frontier path: it rechecks parent and child document hashes around callbacks, retains exact atomic catalog-part/color witnesses and immutable lineages, uses shared aggregate candidate and narrowing ledgers, and admits a family-only decision without falsely settling its ambiguous descendants. The separate `RealBuildPanelCameraRegistration` boundary validates, copies, and deeply freezes hand, determinant, registering panel, turn, and pixel translation as panel-local state. Its pure resolver detaches, canonicalizes, freezes, and verifies the claimed document before reservation; copies built and excluded masks; checks shared-ledger state around the hash, reservation, and each renderer callback; requires a registration panel strictly after the prefix; and reserves exactly eight hypotheses atomically before any seed or render. The empty root yields eight unregistered angular seeds with null observation and shift, while a nonempty prefix retains all attempts and successful observations under a stable hash-bound candidate, unique lineage IDs, and the supplied parent lineage. A cross-hand tie remains unresolved, and a thrown or malformed render preserves attempts and successful counterevidence but authorizes no selection. Arrow derivation binds raw measurement and fitted view to the same panel, computes q0 once, then applies the exact integer D4 map in original family order while preserving travel and off-line tolerance measurements. Every resolver result says that silhouette is not physical-frame authority. The separate fixed-action seam returns `fixed-ledger-frame-unresolved` for every current decision, including a well-shaped caller-supplied proper identity claim, and intentionally never reads the executor; only a future trusted audit producer could mint its opaque authority. No runner, scoring, report, farther-frontier, or fixed-action call site uses these prerequisites yet. The exact attested step-5 shortcut proved that panel 7 separates the two retained origins by `0.12017982101040214`, but its selected eight-piece document remains a diagnostic candidate because finalization's deterministic catalog-realization audit proves that no proper upright target frame survives the step-3 `6106`. The next ordinary carry retains eight lineages from three of four step-6 origins at 8,037 renders and refuses the next 599-render request before work; generic ancestor reversal, arbitrary-depth carry, and reversal-depth reporting remain unbuilt in the driver.
