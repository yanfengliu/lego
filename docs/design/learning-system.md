...
The development test namespace has a deliberately weaker bootstrap contract before production signing and replay-closure certification exist. `TestRunBundleManifestV1` binds exactly two retained roles—the canonical maker request and authority-free maker output—to their content-addressed bytes and non-diagnostic source events, embeds the deterministic downstream capture checkpoints, and is anchored atomically by a `draining → exhausted` event over the exact preceding event count and root. Recording requires explicit artifact-retention consent, local-only provider transmission, no user reference artifacts, and no knowledge, benchmark, or training reuse; every distinct CAS payload is counted against the captured brief's hard `maxStoredBytes` ceiling before its put. The recorder binds the exact capture-checkpoint bytes to the maker terminal event before recording candidate events, so a crash retry cannot replace them. Candidate checkpoints preserve an exact failure stage and code so generation, compilation, validation, and deduplication outcomes cannot be inferred from nullable hashes, but the untrusted unsealed capture may create only neutral `received → archived` ledger records; only a released trusted replay verifier may later record compilation or hard-valid states. The manifest and handle remain visibly `integrity: unsealed` and `authenticated: false`; they carry no seal, replay level, physical claim, promotion evidence, or production authority. Missing population, render, truth-bundle, source, or environment bytes still prevent a complete replay-closure claim.
