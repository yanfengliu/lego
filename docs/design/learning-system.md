...
| Camera horizontal-hand registration | Pure primitive and synthetic tests implemented | The primitive ranks all eight hand-and-quarter-turn silhouette hypotheses, gives each its own registration translation, and refuses exact cross-hand ties; the real-booklet driver, candidate IDs, arrow evidence, fixed-ledger actions, and retained lineage do not consume that frame state. |
