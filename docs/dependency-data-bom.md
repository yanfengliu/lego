...
Status: Gate 0 baseline established 2026-07-10; reconciled with catalog `/12` on 2026-08-10
...
Real LDraw geometry has been bundled since `builtin.basic-parts/7`: 12 parts now carry the expanded triangles of their official LDraw closure as their render mesh, under CC BY 4.0 with per-file authorship preserved. Four existing parts moved from approximate parametric surfaces to exact LDraw surfaces at `/12` while retaining their reviewed connector and conservative collision recipes. Three mesh-backed parts admitted at `/8` take their underside clutch cells from the CC BY-SA 4.0 LDCad Shadow Library because no LEGO Builder record exists for their designs and LDraw geometry cannot supply a female connector. The attribution both licences require is reproduced in [bundled-geometry-notices.md](bundled-geometry-notices.md). Reuse is not training, and that right stays unheld for both.
  ...
  "dataAssets": [
    {
      ...
      "intent": "Project-authored dimensions and parametric box, shell, wedge, compound-box, and analytic-plan recipes cover 77 of the 85 builtin parts. Seventy-three use those recipes for rendering; four moved to exact LDraw render surfaces and visual bounds at builtin.basic-parts/12 while retaining their reviewed project-authored connectors, allowances, and conservative collision recipes. The other eight parts were already declared from measured sources. No LDraw mesh file is copied into this layer; all 12 bundled surfaces are recorded separately under ldraw-bundled-part-geometry."
    },
    {
      ...
      "version": "ldraw-complete-2026-07 archive sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; 12 roots, 118 unique files, 25 named authors; 117 CC BY 4.0 and parts/30503.dat CC BY 2.0 or CC BY 4.0 with the CC BY 4.0 option selected; catalog builtin.basic-parts/12",
      ...
      "intent": "The expanded BFC-corrected triangles of the 12 roots listed in docs/bundled-geometry-notices.md are bundled in immutable asset-local LDraw frames, and each part applies its explicit source-to-catalog frame exactly once. The eight parts admitted at builtin.basic-parts/7 and /8 use separately measured connector and collision data. The four render promotions at /12 also take exact source-derived visual bounds while preserving their prior connector, allowance, and conservative collision declarations; the mesh cannot certify physical semantics. Per-file authorship, title, licence, status, and content hash are preserved for all 118 unique files and reproduced in the generated notice, which packages/catalog/src/bundled-geometry-notices.test.ts holds to the catalog. Both source tables are emitted from the byte-pinned archive, so the triangles and attribution cannot describe different files. Reuse is not training: trainingUseAllowed is false on every bundled record and that right stays unheld."
    },
    {
    ...
    },
    {
      ...
      "rightsPolicy": "project-mit",
      "allowedRoles": ["runtime", "tests", "distribution"],
      "intent": "Disposable Three.js scenes derived from canonical catalog declarations: 73 parts render project-authored parametric primitives and analytic features, while 12 mesh-backed parts render their separately attributed bundled LDraw surface. Rendering is never an authoring source of truth."
    },
    {
...
- The 85-part `builtin.basic-parts/12` catalog, connector taxonomy, collision model, and derived Three.js geometry are implemented with versioned provenance; individual geometry recipes carry SHA-256 content hashes and the truth snapshot pins their aggregate interpretation inputs. Synthetic benchmark examples remain an intent record.
- Real LDraw part geometry is bundled under the selected CC BY 4.0 option for 12 parts: eight admitted at `builtin.basic-parts/7` and `/8`, plus four existing parts promoted to exact visible surfaces and visual bounds at `/12`. File-level attribution covers 117 files that declare CC BY 4.0 plus `parts/30503.dat`, which declares CC BY 2.0 or CC BY 4.0 and is used here under its CC BY 4.0 option. The four promotions preserve their reviewed connector, allowance, and conservative collision declarations; a render mesh does not establish physical truth.
