...
- The build player's `output/booklet/player/<set>/model.mpd` holds official LDraw library files and LEGO Builder stand-in meshes, recorded under `build-player-local-geometry`. It stays local: Git ignores it, only the dev server on 127.0.0.1 serves it, and the production bundle guard refuses it in `apps/web/dist`. Shipping it would need a new BOM record and a notice carrying each LDraw file's CC BY attribution. The Builder stand-ins cannot ship under any current authorization.
