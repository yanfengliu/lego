...
```json
{
  ...
  "dataAssets": [
    ...
    },
    {
      ...
      "intent": "Frame source for every parametric part's LDraw-to-catalog frame (catalog = O * ldraw + t), chosen over three alternatives on 2026-09-23. (a) This route measures each official LDraw file: the four upright yaws are tried, a candidate must put the file's full extent exactly on the catalog bounds and every catalog stud on a stud primitive, and candidates the file's own faces cannot tell apart are one placement; 59 rows are derived, and 2 (32062, 3483) are a reviewed choice among measured candidates, recorded in scripts/ldraw-catalog-frame-derivation.mjs. What is committed is measurement under the facts-only policy: an orientation id, three integers, and the root file's path, sha256, title, author, !LDRAW_ORG and licence for attribution; no LDraw text, triangle or excerpt, and the archive stays local. (b) The community ldraw.xml LDD-to-LDraw mapping was rejected: its authorship and licence are not recorded anywhere this repository can pin, and it maps LDD designs, not the catalog's own frames. (c) Extending scripts/builder_ldraw_frame_pins.py was rejected: those pins are LEGO Builder-to-LDraw frames (the LXFML side of the export, not LDraw-to-catalog), exist for 8 designs, and rest on LEGO Builder data held only under the private-noncommercial set-6651557-nine-part-source-pilot record, which retains per-part integer matrices and translations and no Builder bytes. The retired first-50 frame registry (an ignored run file) is no longer read by playback; tools/booklet compares against it only when LEGO_RUN_EVIDENCE=1 and it is present. Check with node scripts/derive-ldraw-catalog-frames.mjs --check, or LEGO_RUN_EVIDENCE=1 npx vitest run scripts/ldraw-catalog-frame-derivation.test.mjs.",
