...
Status: Gate 0 baseline established 2026-07-10; reconciled with catalog `/32` on 2026-09-24
...
The same gate applies `encoded-source-literal-risk-census/2` to every Git-tracked or non-ignored untracked regular file below `apps/`, `packages/`, and `scripts/`; ignored task evidence, `node_modules`, `dist`, and `coverage` are outside that source population, and symbolic links are refused. JavaScript and TypeScript are parsed with the TypeScript compiler and bounded to 4,194,304 bytes and 1,000,000 AST nodes per file; Python, PowerShell, and C# use a bounded raw-literal/comment lexer plus a conservative whole-source overlay so same-quote literals nested inside interpolation cannot escape the census; reviewed non-executable text contributes matching raw runs. The census records each unique normalized-path/source-span maximal atom matching exact `[A-Za-z0-9+/]{31,}={0,2}` in raw string and no-substitution-template bodies, every raw template head/middle/tail fragment, quoted or computed object/class key, JSX text and string attribute, comment, and reviewed non-executable text, independent of use or dataflow; the conservative Python, PowerShell, and C# overlay also counts qualifying executable identifiers. It refuses every non-exempt atom of at least 128 characters. The repository-wide character total is deterministic review telemetry rather than a hard ceiling: green and red `bom:check` results always report total characters, atom count, classification subtotals, atom-class subtotals, leading files, and ledger digest. A whole non-executable comment value matching exact `sha256:<64 lowercase hex>`, `git:<40 lowercase hex>`, or `md5:<32 lowercase hex>` is the only exemption from the `>=128` refusal; those atoms remain in the ledger, atom count, character total, and classification telemetry. The scanner performs no import, dataflow, or module resolution, and its sorted atom ledger and subtotals are deterministic. A separate source-policy guard refuses literal-only binary `+` composition in JavaScript, TypeScript, and Python, plus Python implicit adjacent string literals, when any direct literal leaf contains a census-sized atom; it deliberately leaves dynamic and short composition, quoted examples, method calls such as `.concat` or `.join`, template substitution, conditionals, `%` and `.format` formatting, f-strings, and other evaluation outside this syntactic contract. Twenty-two gratuitously split literals were rejoined before that guard was added. This remains a narrow contiguous-obvious-encoded-literal risk signal, not a proof of non-reconstructibility or absence of embedded source bytes: it intentionally does not detect arbitrary secrets, fragments shorter than 31 characters even when later combined, regular-expression `.source`, runtime generation such as `repeat` or `fromCharCode`, numeric or escape-based reconstruction, JavaScript/TypeScript executable identifiers, or other encodings. The attempted repo-wide terminal-symbol/module resolver and the generated G3 duplicate-literal roster, generator, test, and seven binding modules were rejected and removed. Separately from the census and literal-fragment guard, the retained G2 consolidation gate resolves only the two named canonical field owners: a binding-aware TypeScript program plus Python `ast`/`symtable` analysis discovers every importing consumer, requires the discovered files and all references to equal exact per-file production-role inventories, rejects shadowing or rebinding, and admits a role only through a conservative public-AST outcome/def-use normal form that refuses dead, conditional, swallowed, and bypassed matches. This is a bounded contract for enumerated production shapes, not a repo-wide terminal-symbol resolver, general control-flow graph, or semantic-equivalence proof. Splitting, dot-breaking, escaping, concatenating, or generating a value merely to move it outside the census is a review blocker and cannot support a risk-reduction claim; admissible reductions remove the value, share one complete counted literal, derive it from independently meaningful domain structure without a census-length guard, or move it into a digest-bound external artifact. Two final byte-identical `/2` runs recorded `totalCharacters=330852`, `atomCount=6097`, classification characters `{non-exempt:330852, terminal-comment-metadata:0}`, atom-class characters `{exact-64-lower-hex:240704, exact-40-lower-hex:5640, exact-32-lower-hex:2560, base64-padding:312, identifier-like-alnum:16460, base64-symbol:65176}`, and ledger `sha256:3668a6a8346ba60f472d0d775545193c97728cf4ce173896841a083e43974067`. The surrounding BOM still reports six workspaces, 38 dependency declarations, 24 third-party packages, four internal packages, and 24 data/source records. Every delivery therefore still requires separate staged added-line, blob-size, secret, license, and attribution review; `bom:check` alone does not protect against every fragmentation strategy or establish redistribution or training rights.

Ledger entries, classification subtotals, dependency declarations, and leading-file rows use explicit locale-independent code-unit ordering, so host locale cannot change the serialized census or BOM result.
...
```json
{
  ...
  "rightsPolicies": {
    "attribution-required-facts-only": {
      ...
      "redistribution": "No LDraw file, geometry, mesh, or excerpt is copied into this repository or shipped. What is kept is measurement — a part's stud positions and body extents as numbers, or its LDraw-to-catalog frame — which is then either hand-authored into a project-owned parametric blueprint or written by a reproducible generator into a committed, checkable table (ldraw-interchange-frames.generated.ts).",
    ...
    "attribution-required-bundled-geometry": {
      "licenseEvidence": "The exact `!LICENSE`, `Author`, `!LDRAW_ORG` and SHA-256 of every bundled file are read from the pinned official archive and preserved per file in packages/catalog/src/ldraw-bundled-sources-6651557.ts. Of the 237 unique files in the 45 bundled closures, 233 directly declare CC BY 4.0; parts/2453b.dat, parts/30503.dat, parts/32064a.dat, and parts/3245c.dat declare CC BY 2.0 or CC BY 4.0, and the bundle selects the CC BY 4.0 option for all four.",
      ...
      "redistribution": "Allowed under CC BY 4.0 with attribution. Real LDraw geometry is bundled for rendering and palette previews. Runtime validators do not inspect the mesh triangles directly, but exact source-derived visual bounds are catalog truth and validator inputs. Connector truth remains separately authored. All twenty-nine fully measured definitions carry attributed authoring-time collision height fields derived from their official surfaces; the sixteen render-only promotions preserve independently authored collision. Runtime consumes emitted bounded boxes rather than treating mesh triangles as a collision oracle.",
  ...
  "dataAssets": [
    {
      ...
      "intent": "Project-authored dimensions and parametric box, shell, wedge, compound-box, and analytic-plan recipes remain physical authority for 77 of the 106 builtin parts: 61 remain parametric renders, while four moved to exact LDraw render surfaces and visual bounds at builtin.basic-parts/12 and twelve more at /13 while retaining their reviewed project-authored connectors, allowances, and conservative collision recipes. The other twenty-nine parts are fully measured definitions: the historical /26 catalog had 98 parts and twenty-one fully measured definitions through 78329; /27 added exact 99563, 73230, 35464, and 49307; /28 added only exact suffixed 3245c and 2453b; /29 added exact 10201 and 3245b; /30 adds none. No LDraw mesh file is copied into this layer; all 45 bundled surfaces are recorded separately under ldraw-bundled-part-geometry."
    },
    {
      ...
      "version": "ldraw-complete-2026-07 archive sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; 45 roots, 237 unique files, 34 named authors; 233 direct CC BY 4.0 declarations, while parts/2453b.dat, parts/30503.dat, parts/32064a.dat, and parts/3245c.dat are CC BY 2.0 or CC BY 4.0 with the CC BY 4.0 option selected; catalog builtin.basic-parts/32. The intent below retains detailed historical measurements through /26; its 37-root, 211-file, and /26 report-control totals are historical rather than current.",
      "declaredLicense": "CC-BY-4.0; parts/2453b.dat, parts/30503.dat, parts/32064a.dat, and parts/3245c.dat are CC-BY-2.0 OR CC-BY-4.0 with CC-BY-4.0 selected",
      ...
      "currentCatalogHistory": "Historical /27 had 41 official roots/closures and 224 unique files and added exact roots parts/99563.dat (sha256 c2706e2934e9fd6f4ba07a7155f83952ad5937c3b4f187ea903a3148b045307e), parts/73230.dat (sha256 ec023f3811316edc70dd0422b1c87ed2630cc299b63382168076947414a3deaf), parts/35464.dat (sha256 0c4a292f9040030ca40a705c617f22a12357975f47dc18d3aaa1b353aa05966b), and parts/49307.dat (sha256 5ab186e08cf7dfd9ff7ce2e1afcd69d08479fd44972340d53d78f67ffdceb135). Historical /28 had 43 roots/closures and 228 unique files and added only exact suffixed roots parts/3245c.dat (sha256 9cc04bca1050e0b16a00c76a432f9cbb769655dc79188a0df3cffa2229dc6615; ten-file closure) and parts/2453b.dat (sha256 3c7caddd91a2467243cf3f3bd0bdd8ee8fa1cbd5435591a6491841ee1ca49b01; six-file closure). Current /30 has 45 roots/closures and 237 unique files; /29 added exact root parts/10201.dat (sha256 028bc441268df93c08a363d375406cdf1eb70a25250bb3b56945fd6828395b7e; 21-file, 18,905-byte closure sha256 3c786b7ef3c89032ab9e4568f53e1962b06987c9cda780c399f752721a4e4a24) and exact root parts/3245b.dat (sha256 3741551acda207402f56b5f7905f1ccc507f8261ea92359fec7829b464b08649; 11-file, 10,868-byte closure sha256 9fdf84fa4dac343eaa9f4f3f30950044ae86613d66033b5c4bffd20a46b139c1). The 10201 redirect to 2436b remains closure evidence rather than a second catalog alias. No bare or cross-suffix identity, Builder identity, printed identity, frame, placement, execution, acceptance, or completion authority follows.",
    ...
    {
      "id": "ldraw-official-interchange-frame-measurements",
      "category": "external-geometry-and-catalog-data",
      "status": "measured-facts-committed-no-geometry-bundled",
      "source": "packages/catalog/src/ldraw-interchange-frames.generated.ts, generated by scripts/derive-ldraw-catalog-frames.mjs from the local byte-pinned official LDraw archive ldraw-complete-2026-07.zip",
      "version": "ldraw-complete-2026-07.zip 144722356 bytes sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; one row per parametric catalog part, 61 rows at builtin.basic-parts/30",
      "declaredLicense": "PER-FILE-CC-BY-2.0-OR-CC-BY-4.0",
      "rightsPolicy": "attribution-required-facts-only",
      "allowedRoles": ["catalog interchange-frame truth", "LDraw import and export", "booklet reference playback", "tests"],
      "intent": "Frame source for every parametric part's LDraw-to-catalog frame (catalog = O * ldraw + t), chosen over three alternatives on 2026-09-23. (a) This route measures each official LDraw file: the four upright yaws are tried, a candidate must put the file's full extent exactly on the catalog bounds and every catalog stud on a stud primitive, and candidates the file's own faces cannot tell apart are one placement; 59 rows are derived, and 2 (32062, 3483) are a reviewed choice among measured candidates, recorded in scripts/ldraw-catalog-frame-derivation.mjs. What is committed is measurement under the facts-only policy: an orientation id and three integers; how the frame was reached (basis, the number of measured candidates, and for a reviewed choice its why); the root file's path, sha256, byte count and closure file count; and title, author, !LDRAW_ORG and licence for attribution. For an official \"~Moved to\" redirect (3023.dat) those four come from the part it moved to (3023b.dat), whose id, sha256 and byte count the row also records. No LDraw text, triangle or excerpt is committed, and the archive stays local. (b) The community ldraw.xml LDD-to-LDraw mapping was rejected: its authorship and licence are not recorded anywhere this repository can pin, and it maps LDD designs, not the catalog's own frames. (c) Extending scripts/builder_ldraw_frame_pins.py was rejected: those pins are LEGO Builder-to-LDraw frames (the LXFML side of the export, not LDraw-to-catalog), exist for 8 designs, and rest on LEGO Builder data held only under the private-noncommercial set-6651557-nine-part-source-pilot record, which retains per-part integer matrices and translations and no Builder bytes. The retired first-50 frame registry (an ignored run file) is no longer read by playback; tools/booklet compares against it only when LEGO_RUN_EVIDENCE=1 and it is present. Check with LEGO_RUN_EVIDENCE=1 node scripts/derive-ldraw-catalog-frames.mjs --check, or LEGO_RUN_EVIDENCE=1 npx vitest run scripts/ldraw-catalog-frame-derivation.test.mjs; without the opt-in both say they did not run, because a clean clone lacks the archive.",
      "trainingUse": "Not designated as a model-training or benchmark corpus; permission to read and measure the files is not permission to train on them."
    },
    {
      ...
      "intent": "Potential interoperability reference. Any future import must preserve file-level source, license, and attribution rather than flattening geometry into project-owned data. Local, uncommitted uses of the pinned LDraw archives have their own records, such as build-player-local-geometry."
    },
    {
      "id": "build-player-local-geometry",
      "category": "external-render-geometry",
      "status": "local-playback-only-private-source-authorized-not-committed-not-bundled",
      "source": "The byte-pinned local official LDraw archive ldraw-complete-2026-07.zip (default C:/tmp/ldraw-complete-2026-07.zip, or LEGO_LDRAW_OFFICIAL_ARCHIVE) and each set's local LEGO Builder native mesh pack (for 21066 C:/tmp/lego-21066-builder-native-part-pack.json, or LEGO_BUILDER_NATIVE_PACK), read by tools/booklet/player-library.ts and packed by tools/booklet/player-model.ts and tools/booklet/player-stage.ts into the ignored output/booklet/player/<set>/model.mpd",
      "version": "Stage lego.booklet-player/1; official archive 144722356 bytes sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; 21066 Builder native pack file sha256 e5bb745faa79c5e7cb525eb0a11a8443815a0c4805c85644204b26c462ac636d, the 107-record pack also recorded under set-6651557-nine-part-source-pilot, with binarySha256 76830eb4832492e5416ad6920ab4f8167b6cf55725641cce162ac8f9f215b6c7; for 21066, 526 official LDraw files, 322 colour lines and six Builder stand-ins covering 32 parts",
      "declaredLicense": "LDRAW-PER-FILE-CC-BY-2.0-OR-CC-BY-4.0; USER-AUTHORIZED-PRIVATE-NONCOMMERCIAL-BUILDER-REFERENCE",
      "rightsPolicy": "private-noncommercial-source-reference",
      "allowedRoles": ["local build-player render geometry", "local reference-build playback served only by the dev server on 127.0.0.1"],
      "intent": "`npm run booklet` writes each set's build-player model to the ignored output/booklet/player/<set>/model.mpd. It embeds every official LDraw part, subpart and primitive the set's official model reaches. Each file's type-1 references are rewritten to library paths, and its own header lines are kept, author, !LDRAW_ORG and !LICENSE included. For 21066 that is 526 official files: 511 declare CC BY 4.0, and 15 declare both CC BY 2.0 and CC BY 4.0. The main model also carries the 322 `0 !COLOUR` lines of the archive's LDConfig.ldr, whose author is LDraw.org and which has no licence line. Its part rows, poses and export sub-model come from LEGO's official 21066 model in ignored output/official-model/ (LXFML sha256 c0564fd86ede633f6cb18738f999fbb70ee948ba93a55cc8d338b4b5f02b5922, LDraw export sha256 096b78037ef1ee15a6dcff90b38f00f09465d0f5a246cb6f5f08fac087dd7bc2), a private reference build under docs/policies/local-rules.md. The six designs the official library lacks are stand-ins built from LEGO Builder meshes: 6801 x8, 7236 x4, 7302 x6, 7562 x12, 8172 x1 and 89680 x1, 32 parts in all. Each stand-in is written as parts/builder/<design>.dat, and its header labels it a stand-in converted from a LEGO Builder mesh. That geometry is LEGO's own, used only locally under the owner's private, noncommercial authorization. The stage refuses an official archive whose size or SHA-256 differs from the pin. It checks the Builder pack's binary against the pack's own binarySha256 but does not pin the pack file; stamp.json records that file's SHA-256. The whole output/ root is gitignored, so model.mpd is never committed. Only the Vite dev server serves it, on 127.0.0.1, through the dev-only middleware in tools/player/serve.ts. scripts/check-production-bundle.mjs, run by npm run build, refuses any .mpd, .ldr, .dat or .pdf file in apps/web/dist, and any file there, text or binary, whose bytes name output/booklet/player, official-model or recipes/. Nothing here is bundled, packaged, uploaded or redistributed.",
      "trainingUse": "Not designated as a model-training or benchmark corpus; the per-file LDraw CC BY terms license reuse rather than training, and the owner's private-noncommercial authorization for the Builder meshes is not training permission."
    },
    {
    ...
    },
    {
      ...
      "source": "Local checksum-pinned LEGO Builder Shell meshes for 43 retained exact design revisions, current catalog/geometry evidence extending the first-50 registry to 66 exact aliases, the retained 21066 LXFML model and derived LDraw MPD, the exact Builder manifest/cache/audit/native-pack evidence join, and local checksum-pinned official/unofficial LDraw archives used only as an independent surface comparison; TypeScript design tuples are declared in apps/web/e2e/real-build-builder-sources.ts, Python design tuples in scripts/builder_calibration_sources.py, and Python Shell identities in scripts/builder_calibration_shell_pins.py",
      "version": "lego.builder-canonical-calibration/8 over frame evidence protocol builder-anchor-frame-plus-ldraw-surface/4 at builtin.basic-parts/29; model sha256 c0564fd86ede633f6cb18738f999fbb70ee948ba93a55cc8d338b4b5f02b5922; derived LDraw MPD 139649 bytes sha256 096b78037ef1ee15a6dcff90b38f00f09465d0f5a246cb6f5f08fac087dd7bc2; 43 reviewed Builder bundle and Shell canonical SHA-256 pairs; TypeScript/Python parity 43 of 43 design tuples and 43 of 43 Shell identities with ordered Shell aggregate sha256 ce023de75e9c5214cd49ebc381e1842cdeb4bd75c6da39b055a11124f5dcd136; official LDraw archive sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; unofficial LDraw archive sha256 09ec08007203b66e79b1f857aa4804cbee26e1337e177a7c3a87adc1268e44d4; exact 184-file official LDraw closure manifest sha256 72ca520b68934fdaa384e9bbc961090538f0b4ee1269773675db1adcf3cc7fdd and parity 184 of 184 files; ignored geometry bundle 1820412 bytes sha256 7e91e1402f2ab609fee6e502336f86ee74fb3a94d970e9b0b75acf07f925a76f; ignored calibration report 54993 bytes sha256 69555bf4a0b7a7beaeb4f98b6d0e5750fa28b6946a957cdc5772633602636a70; ignored proper-world diagnostic /2 78884 bytes sha256 6a26f70df0aa6faac4361a195bd2d95931f8f46acd2e56ecc7c7f052ea0aa940; ignored official XML/LDraw world proposal 764234 bytes sha256 24c10640f118d2961dd297cff608b6978bd54eab85a37cf0c314f4711612f960; ignored exact frame registry 330415 bytes sha256 bcf9702150b73cab1bd70d7ecd0bf33b3b3917522ce4f0ca892be56424b861a1; ignored occurrence/world reconciliation 651618 bytes sha256 4037ecb9cc60bc63bae38b963abeef8096d7405f2da80a40e79fe60fdff4092b; ignored structural-return artifact 7292 bytes sha256 ea1ee9791575ecd858cf13b076d0b3c6de4ebfca9a51a268a9242d3e07667fe3",
      "declaredLicense": "USER-AUTHORIZED-PRIVATE-NONCOMMERCIAL-REFERENCE; LDRAW-PER-FILE-CC-BY-2.0-OR-CC-BY-4.0",
      "rightsPolicy": "private-noncommercial-source-reference",
      "allowedRoles": ["local render-surface comparison", "private diagnostic Builder-to-catalog local-frame derivation", "independent first-50 source and local/world usability census", "authority-absent exact XML-to-derived-LDraw world-transform proposal", "exact first-50 LDraw-to-catalog frame registry", "three-opaque-input first-50 occurrence/world reconciliation", "source-bound first-50 subbuild-completion evidence", "tests with synthetic committed fixtures"],
      "intent": "scripts/extract-builder-shell.py parses exact checksum-pinned Builder bundles into local inspections, and scripts/generate-builder-calibration.py parses already-hashed local archives under fixed ZIP expansion bounds into an ignored geometry bundle. Every traversed LDraw member must match the metadata-only closure's normalized path, SHA-256, Author, !LICENSE, and !LDRAW_ORG values; all 184 are official-library files, 180 declaring CC BY 4.0 and four declaring both CC BY 2.0 and CC BY 4.0, and the unofficial archive may not contribute. scripts/builder_calibration_prefix_contract.py independently binds retained rows to exact manifest/cache/audit/native-pack evidence and recomputes role-bound anchors; only Builder families 0/1 are recognized top studs and family 15 an underside clutch, while an unmapped authored lattice blocks surface-only fallback. A focused parity gate exact-compares all 43 independently committed TypeScript/Python design tuples, all 43 Python Shell identities against their TypeScript source rows, and all 184 canonical LDraw closure files; equality does not turn either language representation into generated or shared authority. The retained 43-row diagnostic precursor covers 197 occurrences and remains counterevidence. Current catalog /29 calibration is published only after explicit semantic coverage verification. The current bounded proposal retains all 320 first-50 rows; the frame compiler binds 66 exact aliases and all 320 occurrences with zero exclusions; the reconciler derives 309 direct plus eleven MultiBuild rows from opaque proposal, frame-registry, and action-preparation inputs with zero quarantine; and the structural compiler binds the official zero-piece step-44 return over occurrences 258 through 280. Current and historical artifacts fail closed when missing, stale, relabeled, or shaped by callers. Each generator verifies reviewed bytes, never overwrites differing current bytes, and requires explicit promotion before a reviewed candidate becomes current evidence. Authority-free selected-path diagnostic /1 commits the first locally complete order selected at each printed step, backtracks only within the current step, never revisits an earlier step choice, and charges its node budget cumulatively across the prefix. On that selected committed prefix it completes printed step 28 with 163 compiled parts and reports selected-committed-prefix-within-step-blocker at printed step 29 occurrence 165 on builtin:jumper-plate-1x2. An abstract isolated search-helper [1,2] versus [2,1] control models the early-return false-negative class; its synthetic tile topology is not a production-valid compiled model, so it does not prove a currently reachable production case. Independently, the implementation has no cross-step backtracking, so the selected-path result cannot support a globally first blocker or infeasibility claim. A successful exact compilation would remain an existence witness, while a global-negative claim requires a separately measured continuation solver. Canonical /2 remains unavailable. Retained 2026-08-28 output records source [280,-36,-124]/upright-yaw-270, target [-280,-24,70]/upright-yaw-270, 749 enumerations, 256 search nodes, and 188.80 seconds; these historical selected-path fields are not a canonical completion result. The earlier remaining-only dead-state memo result is unsound counterevidence because it merged distinct worlds, so none of this establishes step 50. No Builder bundle, Shell report, model XML, LDraw source text, expanded LDraw geometry, generated geometry bundle, calibration report, frame/reconciliation/structural artifact, world-proposal artifact, or source path is committed, packaged, fetched at runtime, uploaded, or approved for training. This evidence grants no official world placement, Builder source execution, prepared-run, trusted action ledger, replay, document mutation, acceptance, or completion authority; it does not admit a PartDefinition or independently certify connector truth (C), collision truth (X), or canonical world frame truth (F), and changing a digest or limit is not source authorization.",
    ...
    },
    {
      ...
      "currentCatalogHistory": "The historical /26 catalog carried thirteen exact LDCad routes. /27 added four exact routes: 99563 via parts/96910.dat; 73230 via p/axlehol4.dat, p/stud2.dat, and parts/73230.dat; 35464 via parts/35464.dat; and 49307 via parts/49307.dat. Historical /28 added exact 3245c via p/stud.dat, parts/3245a.dat, parts/3245c.dat, and parts/s/3245cs01.dat, plus exact 2453b via p/stud.dat and parts/2453b.dat, bringing that historical total to nineteen. /29 added exact 10201 and 3245b routes, bringing the current total to twenty-one. The 10201 redirect to 2436b remains closure evidence rather than a second public alias, and the checked female one-cap A6x44 declaration in the 3245b route emits exactly one `blindAxleHole` endpoint with its finite span and one admitted mouth side; it does not emit a generic through `axleHole`. The intent below retains route history only through /26. Exact connector measurements add no bare or cross-suffix alias, Builder or printed identity, physical frame, placement, replay, execution, acceptance, or completion authority.",
...
- The current 106-part `builtin.basic-parts/32` catalog, connector taxonomy, collision model, and derived Three.js geometry are implemented with versioned provenance; 77 definitions retain project-authored physical recipes and 29 are fully measured, while the render path comprises 61 parametric definitions and 45 exact LDraw roots or closures, including 16 render-only promotions. The historical `/26` catalog contained 98 parts, and `/27` contained 102. Individual geometry recipes carry SHA-256 content hashes and the truth snapshot pins their aggregate interpretation inputs. Synthetic benchmark examples remain an intent record.
- Real LDraw part geometry is bundled under the selected CC BY 4.0 option for 45 parts. Historical `/26` comprised 37 roots, 211 unique files, and 21 fully measured definitions; `/27` added fully measured `99563`, `73230`, `35464`, and `49307`, reaching 41 roots, 224 files, and 25 fully measured definitions; `/28` added exact suffixed `3245c` and `2453b`, reaching 43 roots, 228 files, and 27 fully measured definitions; `/29` added exact `10201` and `3245b`, reaching 45 roots, 237 files, and 29 fully measured definitions. File-level attribution covers 233 direct CC BY 4.0 declarations plus `parts/2453b.dat`, `parts/30503.dat`, `parts/32064a.dat`, and `parts/3245c.dat`, whose CC BY 4.0 option is selected from dual declarations, across 34 named authors. All sixteen render-only promotions preserve their reviewed connector, allowance, and conservative collision declarations; a render mesh does not establish physical truth.
- **Nothing is approved for training.** Reuse of the bundled geometry is not permission to train on it; that right stays unheld and is recorded as `trainingUseAllowed: false` on every record. No LEGO Builder bundle, mesh or XML, LDCad Shadow Library file, research-model code/data/weights, internet-curated model, or booklet page is approved for packaging either. Minimal source-pinned measurements and metadata for the private reconstruction are allowed where this BOM names their exact role; currently that includes official LDraw dimensions, the metadata-only 6651557 LDraw source-resolution audit and 21066 source-coverage ledger, the local-only nine-part LDraw/Builder source pilot, the fifteen-design Builder Shell/frame calibration, the 80015 Builder connector fact and its LDCad cross-check, the local-only LDCad shadow-library connector measurement, and the local booklet edge fixture.
- The build player's `output/booklet/player/<set>/model.mpd` holds official LDraw library files and LEGO Builder stand-in meshes, recorded under `build-player-local-geometry`. It stays local: Git ignores it, and only the dev server on 127.0.0.1 serves it. The production bundle guard (`scripts/check-production-bundle.mjs`) refuses any .mpd, .ldr, .dat or .pdf file in `apps/web/dist`, and any file there that names a player-data path. Shipping it would need a new BOM record and a notice carrying each LDraw file's CC BY attribution. The Builder stand-ins cannot ship under any current authorization.
- The LDCad Shadow Library is CC BY-SA 4.0, and the owner directed on 2026-08-05 that licence must not block this private, noncommercial work, so its derived connector positions are admitted through `builtin.basic-parts/26`, `/27`, `/28`, and `/29`, giving 21 exact current routes. Historical `/26` carried 13 exact routes; `/27` added four for `99563`, `73230`, `35464`, and `49307`; `/28` added exact suffixed `3245c` and `2453b`; `/29` added exact `10201` and `3245b`. Three things that decision did not waive are still true and still recorded: attribution travels with the derived data and is rendered into the notices from the catalog, ShareAlike — including the sui generis database-rights clause — would attach to that derived data on redistribution and a public release must license it share-alike or re-derive it independently, and training rights remain unheld. No shadow file is committed; what is admitted is derived positions.
