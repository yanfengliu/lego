...
Status: Gate 0 baseline established 2026-07-10; reconciled with catalog `/13` on 2026-08-10
...
Real LDraw geometry has been bundled since `builtin.basic-parts/7`: 24 parts now carry the expanded triangles and source-faithful hard-edge normals of their official LDraw closure as their render mesh, under CC BY 4.0 with per-file authorship preserved. Four existing parts moved from approximate parametric surfaces to exact LDraw surfaces at `/12`, and twelve more did so at `/13`; the same `/13` pass corrected LDrawLoader-exact triangulation for 23 generated meshes and source normals for all 24. The `/12` → `/13` migration reports render-geometry changes for all 24 because `54200` also moves from its parametric catalog drawing to its exact mesh. All sixteen promotions retain their reviewed connectors, allowances, and conservative collision recipes. Three mesh-backed parts admitted at `/8` take their underside clutch cells from the CC BY-SA 4.0 LDCad Shadow Library because no LEGO Builder record exists for their designs and LDraw geometry cannot supply a female connector. The attribution both licences require is reproduced in [bundled-geometry-notices.md](bundled-geometry-notices.md). Reuse is not training, and that right stays unheld for both.
...
```json
{
  ...
  "rightsPolicies": {
    ...
    "attribution-required-bundled-geometry": {
      ...
      "redistribution": "Allowed under CC BY 4.0 with attribution. Real LDraw geometry is bundled for rendering and palette previews. Runtime validators do not inspect the mesh triangles directly, but the exact source-derived visual bounds admitted beside them are catalog truth and validator inputs. Connector and collision truth remain independently authored.",
  ...
  "dataAssets": [
    {
      ...
      "intent": "Project-authored dimensions and parametric box, shell, wedge, compound-box, and analytic-plan recipes cover 77 of the 85 builtin parts. Sixty-one use those recipes for rendering; four moved to exact LDraw render surfaces and visual bounds at builtin.basic-parts/12 and twelve more at /13 while retaining their reviewed project-authored connectors, allowances, and conservative collision recipes. The other eight parts were already declared from measured sources. No LDraw mesh file is copied into this layer; all 24 bundled surfaces are recorded separately under ldraw-bundled-part-geometry."
    },
    {
      ...
      "version": "ldraw-complete-2026-07 archive sha256 6009f2e94204c4d3a63a4c812010b5c90bad8c5acb19b882c859fdac63734eae; 24 roots, 163 unique files, 27 named authors; 162 CC BY 4.0 and parts/30503.dat CC BY 2.0 or CC BY 4.0 with the CC BY 4.0 option selected; catalog builtin.basic-parts/13",
      ...
      "allowedRoles": ["runtime render mesh", "palette preview", "visual bounds", "tests", "distribution"],
      "intent": "The expanded BFC-corrected triangles and LDrawLoader-equivalent hard-edge normals of the 24 roots listed in docs/bundled-geometry-notices.md are bundled in immutable asset-local LDraw frames, and each part applies its explicit source-to-catalog frame exactly once. Catalog /13 corrects source-exact triangulation for 23 generated meshes and source normals for all 24; migration reports render geometry for all 24 because 54200 also replaces its parametric /12 drawing. Bounds remain source-derived and unchanged for the twelve already-bundled definitions. The eight parts admitted at builtin.basic-parts/7 and /8 use separately measured connector and collision data. The sixteen in-place render promotions at /12 and /13 take source-derived visual bounds while preserving their prior connector, allowance, connector-grid-centre, partial-overhang-evidence, and conservative collision declarations; the mesh cannot certify physical semantics. Per-file authorship, title, licence, status, and content hash are preserved for all 163 unique files and reproduced in the generated notice, which packages/catalog/src/bundled-geometry-notices.test.ts holds to the catalog. Both source tables are emitted from the byte-pinned archive, so the triangles, normals, and attribution cannot describe different files. Reuse is not training: trainingUseAllowed is false on every bundled record and that right stays unheld."
    },
    {
    ...
    },
    {
      ...
      "rightsPolicy": "project-mit",
      "allowedRoles": ["runtime", "tests", "distribution"],
      "intent": "Disposable Three.js scenes derived from canonical catalog declarations: 61 parts render project-authored parametric primitives and analytic features, while 24 mesh-backed parts render their separately attributed bundled LDraw surface. Rendering is never an authoring source of truth."
    },
    {
...
- The 85-part `builtin.basic-parts/13` catalog, connector taxonomy, collision model, and derived Three.js geometry are implemented with versioned provenance; individual geometry recipes carry SHA-256 content hashes and the truth snapshot pins their aggregate interpretation inputs. Synthetic benchmark examples remain an intent record.
- Real LDraw part geometry is bundled under the selected CC BY 4.0 option for 24 parts: eight admitted at `builtin.basic-parts/7` and `/8`, four existing parts promoted to exact visible surfaces and visual bounds at `/12`, and twelve more promoted at `/13`. The `/13` assets store source-faithful normals for all 24 meshes so type-2 hard edges stay crisp instead of being globally smoothed at runtime, and correct source-exact triangulation for 23 generated meshes. Migration reports render geometry for all 24 because `54200` also replaces its parametric `/12` catalog drawing with that exact mesh. File-level attribution covers 162 files that declare CC BY 4.0 plus `parts/30503.dat`, which declares CC BY 2.0 or CC BY 4.0 and is used here under its CC BY 4.0 option. All sixteen promotions preserve their reviewed connector, allowance, and conservative collision declarations; a render mesh does not establish physical truth.
