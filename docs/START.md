...
- Tests reading ignored run evidence (`output/real-build/`, `output/official-model/`, external archives) skip by default, naming why, and run under `LEGO_RUN_EVIDENCE=1`; opted in, a missing input fails by name ([`scripts/run-evidence-gate.mjs`](../scripts/run-evidence-gate.mjs), [`scripts/run_evidence_gate.py`](../scripts/run_evidence_gate.py)). Booklet-reading Playwright specs also need the opt-in; most skip, saying so, without the booklet.
