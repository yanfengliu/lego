...
2. **Delete the editor UI** (done, milestone 2b, this branch): `/editor.html`, `App.tsx` and everything reachable only from them — `components/`, `viewport/`, `persistence/`, `physics/cart-demo.*`, `automation`, `catalog-search`, `editor-state`, `file-import`, `migration-notice`, `viewport-navigation`, `mesh-part-alignment.test.tsx`, both stylesheets — are gone (59 files, ~8,800 lines). `placement`, `manual-commands` and the connector modules `assembly/` also imports stayed, since deleting them would have broken the kept pipeline. Of the 38 Playwright specs that loaded `/editor.html`: 7 that drove editor UI, needed editor state, or exercised the deleted `physics/cart-demo.ts` were deleted with it; 10 tied to the retired real-build/first-50 family were retired with it (see G3e); 21 that drive kept reading-pipeline code were ported to a new test-only `apps/web/e2e-host.html` (never built, never linked from the player).
...
- [ ] G3e ratchet baseline (today `status/booklet-baseline.json` is reported, not enforced); retire the real-build-* family. Partial: 10 specs that loaded the deleted `/editor.html` were retired with the editor host (2026-09-25) rather than ported — `real-build-step-one-proper-c4-equivariance.spec.ts`, `real-build-step-one-proper-c4-browser-boundary.spec.ts`, `real-build-step-one-silhouette-reuse.spec.ts`, `real-build-farther-depth-narrowing.spec.ts`, `real-build-highlight-compatibility.spec.ts`, `real-build-exact-three-source.spec.ts`, `real-build-browser-output-v4-role-writer.spec.ts`, `real-build-observation-source-parity.spec.ts`, `real-build-observation-source-parity-calibration.spec.ts`, `step1-deferral.spec.ts` (a real-build-family-coupled diagnostic probe). The rest of the family, which never loaded the editor, is untouched and still owes its retirement.
