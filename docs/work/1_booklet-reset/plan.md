...
- [ ] Step 40 `DISCONNECTED_ASSEMBLY`, the first playback failure with all three corrections: 41682's recessed flange face needs a clutch matching the 1x2 plate's studs it takes (or a corrected collision box).
...
- [x] G3b reference build playable in the app, per step (`output/booklet/reference-build.mpd`; step 31's parts are now right under alignment by identity). Open: the editor draws the model mirrored; the sub-build attach-timing gap now cuts it short at step 37/38 too (see above), 2 steps behind playback's own valid-through-39.
