...
The fixed five-panel calibration path can now capture steps/pages 90/79, 101/87, 346/213, 358/218, and 359/219 as exact high/work RGBA, H/P/D stages, independently re-derived W, pairwise P/D/W facts, and lossless PNGs, then replay those bytes in Node. The capture is deliberately unbound to an execution identity and remains pending and authority-free; the separate review foundation still returns `needs-adjudication` for every outcome, and the real-booklet capture, human-owned issuer, publication wrapper, browser-output `/4`, and document authority remain absent.
