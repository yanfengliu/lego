...
A refusal-only local branch semantic inspector now consumes the external branch-role index, exact prepared-run bytes, compiled lineages, typed observation closures, and raw mask roles. It rejects internally orphaned source/camera tables and charges cumulative root groups/identities, search parents/proposals, children, transitions, edges, witnesses/operations, legacy and typed observations, source/camera tables, selection/accepted-transition reference arrays, actual canonical-document byte lengths, and raster work, all before reconstructing any root. It then reconstructs each bounded root once for admission-work measurement, derives and cumulatively charges conservative three-pass compiler work-policy units including step-preparation operations, and only then permits graph/compiler or mask replay. The established public lineage `/1` parser retains its prior semantic contract; the new aggregate gate is explicit on the semantic and bytes-only observation paths, and typed observation reuse requires the exact replay-admitted work inspection rather than semantic validation alone. The inspector integrity-binds prepared step/compiler/proposal facts, requires legacy observation-generation, selection, and acceptance fields to remain empty, separately verifies each typed closure and any nonempty committed raw role, and returns authority-absent summaries. The compiler meters are admission-policy proxies rather than literal CPU or allocation counts, and this is not a complete browser-output `/4` report reader: PDF/page/crop, source/camera, report-order, terminal-document, placement, and completion authority remain absent.

The standalone atomic branch producer now accepts a bounded ordered frontier over multiple exact root documents, gives each retained lineage a digest that commits both its structural lineage and exact canonical parent bytes, keeps exact-byte-convergent identities in one group, resolves each proposal against its own retained parent, proves a worst bounded zero-frontier terminal envelope fits, and reserves the entire cross-root request once before compilation. A separate transport-only finalizer accepts only module-produced batch results, requires a contiguous terminal-respecting prefix, admits aggregate role bytes and cumulative semantic/replay work before raw observation-role copying, rechecks measure/copy digests, verifies optional typed observation bytes, and publishes dense private roles through fresh-copy reads. Both seams capture the storage, serialization and byte-copy intrinsics used after hostile inputs can run. Neither seam is connected to the live runner, a complete browser-output `/4` envelope, an authority issuer, or user-document mutation; public browser output remains `/3`.
