...
- `npm run booklet` reads booklet 6651557, identifies every callout's part, aligns each printed step to the official model of set 21066 by those parts, checks catalog coverage, and replays the official poses through the brick kernel. It prints a per-stage summary and writes `output/booklet/status.json`, plus `reference-build.mpd`: the valid prefix as a labelled reference build. Open it with the editor's Import, then Build, to step through it one printed step at a time. Inputs default to `recipes/6651557.pdf` and `output/official-model/vx1087034_21066_a.xml`, overridden by the env vars `BOOKLET_PDF`, `BOOKLET_LXFML`, `BOOKLET_OFFICIAL_LDRAW`, and `BOOKLET_OUT`. Playback takes every LDraw-to-catalog frame from the catalog. `BOOKLET_LDRAW_FRAMES` names the retired first-50 frame registry, which is compared with the catalog only under `LEGO_RUN_EVIDENCE=1`; nothing waits on it.
