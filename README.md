...
- `npm run booklet` reads booklet 6651557, aligns its printed steps to the official model of set 21066, checks catalog coverage, and replays the official poses through the brick kernel. It prints a per-stage summary and writes `output/booklet/status.json`. Inputs default to `recipes/6651557.pdf` and `output/official-model/vx1087034_21066_a.xml`, overridden by the env vars `BOOKLET_PDF`, `BOOKLET_LXFML`, `BOOKLET_OFFICIAL_LDRAW`, `BOOKLET_LDRAW_FRAMES`, and `BOOKLET_OUT`.
