...
Status: Gate 0/1 implementation is underway and the first Gate 2 harness foundations are landing. The repository now contains a runnable browser studio with integrity-checked local project persistence, strict wire schemas, truth-bound immutable template admission, a project-authored starter catalog, deterministic assembly kernel, a bounded four-candidate maker population, disposable Three.js renderer, exact canonical render-packet validation, a bounded LDraw profile, and the companion package's local content-addressed artifact-store foundation. Provider-backed AI, the authoritative companion broker surfaces, and the recursive engineering runner are not implemented yet.
