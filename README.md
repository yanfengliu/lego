...
Status: Gate 0/1 implementation is underway. The repository now contains a runnable browser studio with integrity-checked local project persistence, strict wire schemas, truth-bound immutable template admission, a project-authored starter catalog, deterministic assembly kernel, disposable Three.js renderer, a bounded LDraw profile, and the companion package's local content-addressed artifact-store foundation. Provider-backed AI, the authoritative companion broker surfaces, and the recursive engineering runner are not implemented yet.
